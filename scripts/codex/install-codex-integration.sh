...
run_repository_skill_generation() {
  ...
  # `skills` feature, the repository root as output, and `delete`, which removes
  # skills no longer projected and makes `--check` report a stale one.
...
check_repository_skills() {
  ...
  if run_repository_skill_generation --check >/dev/null; then
    printf 'ok repository skills generated by %s/rulesync.jsonc\n' "$repo_root"
  else
    printf 'missing-or-drifted repository skills generated by %s/rulesync.jsonc\n' "$repo_root"
    failed=1
  fi
