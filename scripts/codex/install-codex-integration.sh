...
# `.rulesync/mcp.jsonc` is the only MCP source. Rulesync writes each installed
# CLI's user-level MCP file from it and owns that file's server list. Codex is
# not listed: Rulesync's global output always targets ~/.codex/config.toml and
# ignores CODEX_HOME, so the composer merges the Codex projection instead.
user_mcp_clis=(claude:claudecode copilot:copilotcli agy:antigravity-cli)
...
repository_generated_exclude_entries=("/.codex/hooks.json" "/.claude/settings.json" "/.github/hooks/" "/.agents/hooks.json")
...
require_rulesync() {
  [[ -x "$rulesync_bin" ]] && return 0
  printf 'missing-rulesync %s (run pnpm install --frozen-lockfile in %s)\n' "$rulesync_bin" "$repo_root" >&2
  return 1
}

run_repository_outputs_generation() {
  require_rulesync || return 1
  # rulesync.jsonc is the only Rulesync configuration: the four targets, the
  # skills and hooks features, the repository root as output, and `delete`,
  # which removes stale generated files and makes `--check` report drift.
  (cd -- "$repo_root" && "$rulesync_bin" generate --config "$repo_root/rulesync.jsonc" "$@" --silent)
}
...
generate_repository_outputs() {
  local exclude_file
  run_repository_outputs_generation || return 1
  exclude_file="$(repository_skill_exclude_file)" || return 1
  mkdir -p -- "$(dirname -- "$exclude_file")"
  for entry in "$repository_skill_exclude_entry" "${repository_generated_exclude_entries[@]}"; do
...
check_repository_outputs() {
  local failed=0
  local exclude_file
  if run_repository_outputs_generation --check >/dev/null; then
    printf 'ok repository outputs generated by %s/rulesync.jsonc\n' "$repo_root"
  else
    printf 'missing-or-drifted repository outputs generated by %s/rulesync.jsonc\n' "$repo_root"
    failed=1
  fi
  if exclude_file="$(repository_skill_exclude_file)"; then
    for entry in "$repository_skill_exclude_entry" "${repository_generated_exclude_entries[@]}"; do
      if grep -qxF -- "$entry" "$exclude_file" 2>/dev/null; then
...
# The Codex projection of `.rulesync/mcp.jsonc`, generated once per run into a
# temporary root. The composer merges its servers into $CODEX_HOME/config.toml,
# and the role renderer and execution-contract builder read the same file.
generate_codex_mcp_source() {
  require_rulesync || return 1
  (cd -- "$repo_root" && "$rulesync_bin" generate \
    --input-roots "$repo_root/.rulesync" \
    --targets codexcli \
    --features mcp \
    --output-roots "$codex_mcp_source_root" \
    --silent) && [[ -f "$codex_mcp_source" ]]
}

installed_user_mcp_targets() {
  local entry
  local installed=()
  for entry in "${user_mcp_clis[@]}"; do
...
run_user_mcp_generation() {
  local target_list="$1"
  shift
  require_rulesync || return 1
  (cd -- "$repo_root" && "$rulesync_bin" generate --global \
    --input-roots "$repo_root/.rulesync" \
    --targets "$target_list" \
    --features mcp \
    "$@" \
...
generate_user_mcp() {
  local targets
  targets="$(installed_user_mcp_targets)"
  if [[ -z "$targets" ]]; then
    printf 'skipping user-level MCP (no claude, copilot, or agy CLI installed)\n' >&2
    return 0
  fi
  run_user_mcp_generation "$targets"
}
...
check_user_mcp() {
  ...
  targets="$(installed_user_mcp_targets)"
  if [[ -z "$targets" ]]; then
    printf 'skipping user-level MCP check (no claude, copilot, or agy CLI installed)\n'
    return 0
  fi
  if run_user_mcp_generation "$targets" --check >/dev/null; then
    printf 'ok user-level MCP (%s) generated from %s/.rulesync/mcp.jsonc\n' "$targets" "$repo_root"
    return 0
  fi
  printf 'missing-or-drifted user-level MCP (%s) from %s/.rulesync/mcp.jsonc\n' "$targets" "$repo_root"
  return 1
}
...
check_cocoindex_code_config() {
  local failed=0
  grep -Fq '[mcp_servers.cocoindex-code]' "$codex_mcp_source" || {
    printf 'missing-user-mcp-registration cocoindex-code (.rulesync/mcp.jsonc)\n'
    failed=1
  }
  grep -Fq 'run-autodev-mcp.sh\" cocoindex-code' "$codex_mcp_source" || {
    printf 'invalid-user-mcp-args cocoindex-code (.rulesync/mcp.jsonc)\n'
    failed=1
  }
  grep -Fq 'name = "ccc"' "$user_config_portable_source" || {
...
check_links() {
  ...
  if ! check_repository_outputs; then
    failed=1
  fi
  if ! check_user_mcp; then
    failed=1
  fi
...
# Rulesync requires a normalized absolute output root, and TMPDIR often ends in `/`.
codex_mcp_source_root="$(cd -- "$(mktemp -d "${TMPDIR:-/tmp}/autodev-codex-mcp.XXXXXX")" && pwd -P)"
trap 'rm -rf -- "$codex_mcp_source_root"' EXIT
codex_mcp_source="$codex_mcp_source_root/.codex/config.toml"
if ! generate_codex_mcp_source; then
  printf 'could not generate the Codex MCP projection from %s/.rulesync/mcp.jsonc\n' "$repo_root" >&2
  exit 1
fi
