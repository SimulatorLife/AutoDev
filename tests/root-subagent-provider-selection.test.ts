...
const contract = await import(
  "./fixtures/contracts/root-subagent-provider-selection.json",
  { with: { type: "json" } }
...
const seeded = (): (() => number) =>
  mulberry32(Number.parseInt(String(contract.seed), 16));
const shape = (candidate: JsonRecord): JsonRecord => ({
...
describe("root versus subagent provider selection", () => {
  test("keeps the root alias distinct from every leaf alias", () => {
    assert.equal(
      routing.roleForModel(contract.aliases.orchestrator.model),
      contract.aliases.orchestrator.roleForModel
    );
  ...
  test("freezes seeded leaf membership, models, and ordering", () => {
    for (const role of ["default", "smart"]) {
      ...
      assert.deepEqual(
        new Set(
          routing.roleCandidates(role, seeded()).map(({ provider }) => provider)
        ),
  ...
  test("freezes root fallback reasoning and preferred continuation ordering", () => {
    ...
    assert.deepEqual(
      new Set(
        routing
          .orchestratorCandidates(seeded(), "claude")
          .map(({ provider }) => provider)
      ),
      new Set(contract.tiers.orchestrator)
    );
    assert.deepEqual(
  ...
  test("freezes root-only fallback reasoning overrides and leaf effort preservation", () => {
    ...
    assert.deepEqual(
      ...
      contract.payloads.orchestratorFallback
    );
    assert.deepEqual(
      payloadForCandidate(
        { model: "autodev/explorer", reasoning: { effort: "xhigh" } },
        routing.roleCandidates("explorer", seeded())[0]!
      ),
      contract.payloads.subagent
    );
  });
});
