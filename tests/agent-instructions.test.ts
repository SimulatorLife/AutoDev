...
test('CLAUDE.md is a symlink to AGENTS.md', () => {
  assert.equal(lstatSync(join(repositoryRoot, 'CLAUDE.md')).isSymbolicLink(), true);
  assert.equal(readlinkSync(join(repositoryRoot, 'CLAUDE.md')).toString(), 'AGENTS.md');
});

test('no other instruction copies exist', () => {
  for (const relativePath of ['.github/copilot-instructions.md', '.claude/CLAUDE.md', 'GEMINI.md', '.rulesync/rules']) {
    assert.equal(lstatSafe(join(repositoryRoot, relativePath)), false, relativePath);
  }
  ...
  const copies = tracked.filter((relativePath) => {
    ...
    return relativePath !== 'AGENTS.md' && lstatSafe(absolutePath) && !lstatSync(absolutePath).isSymbolicLink() && readFileSync(absolutePath).equals(expected);
  });
