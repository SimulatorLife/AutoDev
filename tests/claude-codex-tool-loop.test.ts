...
function parseSse(text: string): Array<{ event: string; data: JsonRecord | null }> {
...
function completedResponse(sse: Array<{ event: string; data: JsonRecord | null }>): JsonRecord {
...
const CODEX_TOOLS = {
  ...
  tools: [ {
    ...
    description: "",
    tools: [
      { type: "custom", name: "exec", description: "Run JavaScript code to orchestrate/compose tool calls", format: { type: "grammar", syntax: "lark", definition: "start: /.+/" } },
      { type: "function", name: "wait", description: "Wait on an exec cell", parameters: { type: "object", properties: { cell_id: { type: "string" } }, required: [ "cell_id" ] } },
    ],
  } ],
};

function codexRequest(port: number, input: JsonRecord[], signal?: AbortSignal): Promise<Response> {
  return fetch(`http://127.0.0.1:${port}/v1/responses`, {
    method: "POST",
    headers: { "content-type": "application/json", "x-autodev-agent-role": "worker" },
    body: JSON.stringify({ model: "claude-subscription", stream: true, cwd: REPO_ROOT, input }),
    ...(signal ? { signal } : {}),
  });
}
...
test("a Claude turn acts only through Codex tools that Codex executes", async () => {
  const script = [
    ...
    { call: "exec", arguments: { input: "text(await tools.exec_command({ cmd: 'ls' }))" } },
    { say: "Codex ran it: {result}" },
  ];
  await withBridge(script, async ({ port, record }) => {
    const history: JsonRecord[] = [
      CODEX_TOOLS,
      { type: "message", role: "developer", content: [ { type: "input_text", text: "role policy from the worker TOML" } ] },
      { type: "message", role: "user", content: [ { type: "input_text", text: "List the workspace." } ] },
    ];
    ...
    assert.deepEqual(firstResponse.output.map((item: JsonRecord) => item.type), [ "reasoning", "custom_tool_call" ]);
    assert.equal(firstResponse.output[0].summary[0].text, "I should list the files.");
    ...
    assert.equal(first.filter(({ event }) => event === "response.output_item.done").length, 2, "each item is finished as it completes");
    ...
    assert.match(cli.prompt, /<developer>\nrole policy from the worker TOML\n<\/developer>/);
    assert.match(cli.prompt, /<user>\nList the workspace\.\n<\/user>/);
    assert.ok(isAlive(cli.pid), "the CLI stays parked on the call between requests");

    const continuation = [
      ...history,
      call,
      { type: "custom_tool_call_output", call_id: call.call_id, output: [ { type: "input_text", text: "Script completed\nOutput:\n" }, { type: "input_text", text: "README.md" } ] },
    ];
    const second = parseSse(await (await codexRequest(port, continuation)).text());
    const secondResponse = completedResponse(second);
    assert.equal(secondResponse.status, "completed");
    assert.deepEqual(secondResponse.output.map((item: JsonRecord) => item.type), [ "message" ]);
    assert.equal(secondResponse.output_text, "Codex ran it: Script completed\nOutput:\n");
    const finished = await record();
    assert.deepEqual(finished.results[0].content.map((part: JsonRecord) => part.text), [ "Script completed\nOutput:\n", "README.md" ]);
  });
});
...
test("a continuation carrying Codex's own notification resumes the same parked turn", async () => {
  ...
  // see what was added.
  await withBridge([ { call: "exec", arguments: { input: "text(1)" } }, { say: "saw {result}" } ], async ({ port, record }) => {
    const history: JsonRecord[] = [ CODEX_TOOLS, { type: "message", role: "user", content: "go" } ];
    const call = completedResponse(parseSse(await (await codexRequest(port, history)).text())).output.at(-1);
    const { pid } = await record();
    const continuation = [
      ...history,
      call,
      { type: "custom_tool_call_output", call_id: call.call_id, output: "Script completed" },
      { type: "message", role: "user", content: [ { type: "input_text", text: "<subagent_notification>child done</subagent_notification>" } ] },
    ];
    const response = completedResponse(parseSse(await (await codexRequest(port, continuation)).text()));
    assert.equal(response.output_text, "saw Script completed");
    const finished = await record();
    assert.equal(finished.pid, pid, "the parked CLI continued; no fresh CLI was started");
    assert.match(finished.results[0].content.at(-1).text, /Codex added to the conversation while this ran:[\s\S]*<subagent_notification>child done<\/subagent_notification>/);
  });
});

test("a function tool call carries JSON arguments back to Codex", async () => {
  await withBridge([ { call: "wait", arguments: { cell_id: "7" } }, { say: "done" } ], async ({ port }) => {
    const response = completedResponse(parseSse(await (await codexRequest(port, [ CODEX_TOOLS, { type: "message", role: "user", content: "go" } ])).text()));
...
test("a continuation for a turn the bridge no longer holds starts over from the transcript", async () => {
  await withBridge([ { say: "resumed" } ], async ({ port, record }) => {
    const input = [
      ...
      { type: "custom_tool_call", call_id: "call_elsewhere_1", name: "exec", input: "text(1)" },
      { type: "custom_tool_call_output", call_id: "call_elsewhere_1", output: "earlier output" },
    ];
    const response = completedResponse(parseSse(await (await codexRequest(port, input)).text()));
...
test("a client that disconnects mid-turn takes the CLI down with it", async () => {
  await withBridge([ { think: "working" }, { sleep: 30_000 }, { say: "too late" } ], async ({ port, record }) => {
    const controller = new AbortController();
    const response = await codexRequest(port, [ { type: "message", role: "user", content: "go" } ], controller.signal);
    ...
    controller.abort();
    const deadline = Date.now() + 5000;
    while (isAlive(pid) && Date.now() < deadline) await new Promise((resolveSleep) => setTimeout(resolveSleep, 50));
    assert.equal(isAlive(pid), false, "the CLI must not keep working after its client went away");
  });
});
