...
import { accessSync, constants as fsConstants, readFileSync, mkdtempSync, rmSync, writeFileSync } from 'node:fs';
...
function freePort(): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') { server.close(); reject(new Error('could not determine free port')); return; }
...
async function post(port: number, path: string, payload: unknown): Promise<void> {
  const response = await fetch(`http://127.0.0.1:${port}${path}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(payload),
  });
  assert.equal(response.status, 200, `Collector returned HTTP ${response.status} for ${path}`);
}

test('the pinned Collector forwards all signals without prompt logging', async (t: TestContext) => {
  if (!configuredBinary) { t.skip('set AUTODEV_OTELCOL_BIN to the pinned v0.160.0 binary for the live Collector smoke test'); return; }
  const binary = configuredBinary;
  try { accessSync(binary, fsConstants.X_OK); } catch { t.skip(`Collector binary is not executable: ${binary}`); return; }
  ...
  writeFileSync(config, readFileSync(join(repositoryRoot, 'config/otel/collector.yaml'), 'utf8')
  ...
  const receiver = createServer((request: IncomingMessage, response: ServerResponse) => {
    ...
    request.on('end', () => {
      forwarded.push({ path: request.url ?? '', body: Buffer.concat(chunks).toString('utf8') });
      response.writeHead(200, { 'content-type': 'application/json' });
      response.end('{}');
    });
  });
  await new Promise<void>((resolve, reject) => { receiver.once('error', reject); receiver.listen(receiverPort, '127.0.0.1', () => resolve()); });
  const environment = {
    ...process.env,
    AUTODEV_OTELCOL_BIN: binary,
    AUTODEV_OTEL_CONFIG: config,
    AUTODEV_OTEL_VERSION_FILE: join(repositoryRoot, 'config/otel/collector.version'),
    AUTODEV_OTEL_HOST: '127.0.0.1',
    AUTODEV_OTEL_PORT: String(collectorPort),
  };
  const processHandle = spawn(runner, [], { cwd: repositoryRoot, env: environment, stdio: ['ignore', 'pipe', 'pipe'] });
  try {
    await waitForPort('127.0.0.1', collectorPort);
    const fixture = JSON.parse(replaceFixtureTimes(readFileSync(fixturePath, 'utf8'))) as JsonObject;
    for (const [path, key] of [['/v1/logs', 'logs'], ['/v1/traces', 'traces'], ['/v1/metrics', 'metrics']] as const) await post(collectorPort, path, fixture[key]);
    await post(collectorPort, '/v1/metrics', fixture.metrics);
    await new Promise((resolve) => setTimeout(resolve, 1_000));
    assert.deepEqual(forwarded.map((entry) => entry.path).sort(), ['/v1/logs', '/v1/metrics', '/v1/metrics', '/v1/traces']);
  } finally {
