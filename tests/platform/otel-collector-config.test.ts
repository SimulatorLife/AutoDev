...
type Token = { indent: number; isList: boolean; key: string | null; value: string };
type CollectorAsset = { name: string; sha256: string };
type CollectorManifest = { schema: string; version: string; assets: Record<string, CollectorAsset> };
...
const artifactsPath = join(repositoryRoot, 'config/otel/collector-artifacts.json');
...
const UNSUPPORTED_TOP_LEVEL_SECTIONS = ['processors', 'extensions', 'connectors'] as const;
...
function tokenize(text: string): Token[] {
  ...
  for (const rawLine of text.split('\n')) {
    ...
    if (stripped === '-' || stripped.startsWith('- ')) {
      tokens.push({ indent, isList: true, key: null, value: stripped.slice(1).trim() });
      continue;
    }
    const separator = stripped.indexOf(':');
    if (separator < 0) throw new Error(`Cannot parse YAML line: ${rawLine}`);
    tokens.push({ indent, isList: false, key: stripped.slice(0, separator).trim(), value: stripped.slice(separator + 1).trim() });
  }
  return tokens;
}

function parseBlock(tokens: Token[], start: number, indent: number): [YamlValue, number] {
  ...
  while (index < tokens.length) {
    ...
    } else if (token.value.startsWith('[') && token.value.endsWith(']')) child = parseInlineList(token.value);
...
function loadMinimalYaml(text: string): YamlValue {
  ...
  if (index !== tokens.length) throw new Error('Unconsumed YAML content while parsing fixture');
  return value;
}

function asMap(value: YamlValue | undefined): Record<string, YamlValue> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('expected YAML mapping');
  return value;
}
...
describe('OpenTelemetry Collector fixture', () => {
  ...
  test('platform artifact manifest matches the pinned version', () => {
    const manifest = JSON.parse(readFileSync(artifactsPath, 'utf8')) as CollectorManifest;
    assert.equal(manifest.schema, 'autodev-otel-collector-artifacts-v1');
    assert.equal(manifest.version, EXPECTED_VERSION);
    assert.deepEqual(Object.keys(manifest.assets).sort(), ['darwin/amd64', 'darwin/arm64', 'linux/amd64', 'linux/arm64']);
    for (const asset of Object.values(manifest.assets)) {
      assert.match(asset.name, new RegExp(`^otelcol_${EXPECTED_VERSION.slice(1)}_.*\\.tar\\.gz$`));
...
describe('OpenTelemetry Collector config structure', () => {
  test('top-level sections are receivers, exporters, and service only', () => {
    assert.deepEqual(Object.keys(config).sort(), ['exporters', 'receivers', 'service']);
  });

  test('unsupported top-level sections are absent', () => {
    for (const section of UNSUPPORTED_TOP_LEVEL_SECTIONS) assert.equal(Object.hasOwn(config, section), false, section);
  });
  ...
  test('service has pipelines and telemetry sections only', () => {
    ...
    for (const section of UNSUPPORTED_SERVICE_SECTIONS) assert.equal(Object.hasOwn(service, section), false, section);
    assert.deepEqual(asMap(asMap(service.telemetry).metrics), { level: 'none' });
  });
  ...
  test('all three pipelines wire the OTLP receiver to the AutoDev exporter', () => {
    ...
    assert.deepEqual(Object.keys(pipelines).sort(), [...EXPECTED_PIPELINES].sort());
    for (const name of EXPECTED_PIPELINES) {
      const pipeline = asMap(pipelines[name]!);
      assert.deepEqual(Object.keys(pipeline).sort(), ['exporters', 'receivers']);
      assert.deepEqual(asList(pipeline.receivers), ['otlp']);
      assert.deepEqual(asList(pipeline.exporters), [EXPECTED_EXPORTER_NAME]);
      assert.equal(Object.hasOwn(pipeline, 'processors'), false, `${name} must not declare processors`);
    }
  });
});
