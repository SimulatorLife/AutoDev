...
import { ensureRouterAuth, readCollectorMode, writeCollectorMode } from '../../src/platform/install-state.ts';
...
test('Collector mode state defaults safely and rejects symlinks', () => withTempDir((directory) => {
...
test('router auth creates a private token, preserves existing env content, and is idempotent', () => withTempDir((directory) => {
...
test('router auth replaces an empty token entry rather than treating it as configured', () => withTempDir((directory) => {
  const path = join(directory, '.env');
  writeFileSync(path, 'CODEX_ROUTER_AUTH_TOKEN=\n');
  const token = ensureRouterAuth(path, { AUTODEV_SKIP_LAUNCHCTL: '1' });
  assert.match(readFileSync(path, 'utf8'), new RegExp(`CODEX_ROUTER_AUTH_TOKEN=${token}`));
}));
