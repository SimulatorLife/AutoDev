...
import { ensureCollector, resolveCollectorOptions, runCollector, type CollectorOptions } from '../../src/platform/otel-collector.ts';
...
test('Collector run uses an explicit binary and exact version/config', () => withTempDir((directory) => {
  baseFiles(directory);
  ...
  assert.equal(readFileSync(join(directory, 'args'), 'utf8').trim(), `--config\n${join(directory, 'collector.yaml')}`);
}));
...
test('Collector check validates an existing listener without starting a duplicate', () => withTempDir((directory) => {
  baseFiles(directory);
  const binary = fakeBinary(directory);
...
test('Collector run refuses an existing HTTP service as a duplicate', () => withTempDir((directory) => {
  baseFiles(directory);
  const binary = fakeBinary(directory);
  ...
  assert.throws(() => withPath(directory, () => runCollector(options(directory, binary))), /duplicate/);
}));
...
`);
    chmodSync(join(bin, 'curl'), 0o700);
    const collectorOptions = { ...options(directory, join(directory, 'otelcol')), runner, runDir, pidFile: join(runDir, 'collector.pid'), ensureLog: join(runDir, 'collector.log'), startTimeoutSeconds: 3 };
