...
import { CANONICAL_HOOK_HASHES, checkHookTrust, syncHookTrust } from '../../src/platform/install-materializer.ts';
...
test('syncHookTrust populates canonical hashes and strips obsolete config.toml entries', () => withTempDir((dir) => {
...
`;
  ...
  for (const [suffix, hash] of Object.entries(CANONICAL_HOOK_HASHES)) {
    assert.match(updatedContent, new RegExp(JSON.stringify(`${userHooks}:${suffix}`)));
    assert.match(updatedContent, new RegExp(JSON.stringify(`${projectHooks}:${suffix}`)));
    assert.match(updatedContent, new RegExp(hash));
  }
}));

test('checkHookTrust detects missing or drifted hashes', () => withTempDir((dir) => {
  const codexHome = join(dir, '.codex');
  const repoRoot = join(dir, 'AutoDev');
  const configPath = join(codexHome, 'config.toml');
  ...
  const tampered = content.replace(CANONICAL_HOOK_HASHES['user_prompt_submit:0:0'], 'sha256:tampered00000000000000000000000000000000000000000000000000000000');
