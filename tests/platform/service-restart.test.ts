...
import { MANAGED_SERVICE_LABELS, restartServices, type ServiceRestartDeps, type ServiceRestartOptions } from '../../src/platform/service-restart.ts';

function options(overrides: Partial<ServiceRestartOptions> = {}): ServiceRestartOptions {
  return { repositoryRoot: '/repo', home: '/home', codexHome: '/home/.codex', otelMode: 'direct', readyAttempts: 1, readyDelayMs: 0, ...overrides };
}

function deps(overrides: Partial<ServiceRestartDeps> = {}): ServiceRestartDeps & { calls: string[]; runs: { args: readonly string[]; input?: string; env?: NodeJS.ProcessEnv }[] } {
  const calls: string[] = [];
  const runs: { args: readonly string[]; input?: string; env?: NodeJS.ProcessEnv }[] = [];
  return {
    calls,
    runs,
    launchd: {
      isLoaded: () => false,
      print: () => '',
      bootout: (label) => { calls.push(`bootout:${label}`); },
      bootstrap: (plist) => { calls.push(`bootstrap:${plist}`); },
      enable: (label) => { calls.push(`enable:${label}`); },
      kickstart: (label) => { calls.push(`kickstart:${label}`); },
    },
    fileExists: () => true,
    readFile: () => '',
    ...
    probe: async () => true,
    sleep: async () => {},
    run: (_command, args, input, env) => { runs.push({ args, ...(input === undefined ? {} : { input }), ...(env === undefined ? {} : { env }) }); return 0; },
    listeningPids: () => [],
    commandLine: () => null,
    kill: () => {},
    ...overrides,
...
test('launchd restart adopts every configured service and runs direct ensures', async () => {
  ...
  assert.equal(fake.calls.filter((call) => call.startsWith('bootstrap:')).length, 5);
  assert.equal(fake.calls.filter((call) => call.startsWith('enable:')).length, 5);
  assert.ok(fake.calls.includes('bootout:com.codex.otel-collector'));
  assert.equal(fake.runs.length, 5);
  assert.ok(fake.runs.some((run) => run.args.at(-1)?.endsWith('ensure-codex-model-router.sh')));
});
...
test('launchd-unavailable mode forwards Collector paths and runs fallback ensures', async () => {
  const fake = deps({ commandAvailable: () => false, fileExists: () => false, probe: async () => false });
  assert.equal(await restartServices(options({ otelMode: 'collector', readyAttempts: 0 }), fake), 0);
  const collector = fake.runs.find((run) => run.args.at(-1)?.endsWith('ensure-autodev-otel-collector.sh'));
