...
async function withProxy(
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      setTimeout(
        () => reject(new Error("MiniMax proxy did not start")),
        10_000
      ).unref();
    });
    await run(proxyPort);
...
test("only the credential and content negotiation headers leave the machine", async () => {
      ...
      workspaces: {
        "/Users/someone/private-repo": {
          associated_remote_urls: {
            origin: "git@example.invalid:private/repo.git"
          }
        }
      }
    }),
  ...
  const localHeaders = [
    ...Object.keys(routerHeaders),
    ...Object.keys(codexHeaders)
  ].filter(
    (name) => !["accept", "authorization", "content-type"].includes(name)
  );
  ...
  await withProxy(
    ...
    async (proxyPort) => {
      const response = await fetch(
        `http://127.0.0.1:${proxyPort}/v1/responses`,
        {
          method: "POST",
          headers: {
            ...(routerHeaders as Record<string, string>),
            ...codexHeaders
          },
          body: JSON.stringify({
            model: "MiniMax-M3",
            ...
            client_metadata: {
              session_id: "01a0a60e-local-session",
              "x-codex-turn-metadata": JSON.stringify({
                workspaces: { "/Users/someone/private-repo": {} }
              })
            }
          })
        }
  ...
  assert.equal(
    ...
    "Bearer minimax-key",
    "the provider credential must still reach the API"
  );
  assert.equal(upstreamRequest!.headers["content-type"], "application/json");
  for (const name of localHeaders) {
    assert.equal(
      upstreamRequest!.headers[name],
      undefined,
      `${name} must never be forwarded to api.minimax.io`
    );
  }
  assert.equal(
    "client_metadata" in upstreamRequest!.body,
    false,
    "body-embedded turn metadata must not leave the machine"
  );
  assert.equal(JSON.stringify(upstreamRequest).includes("private-repo"), false);
  assert.equal(
    upstreamRequest!.body.prompt_cache_key,
    "cache-key",
    "the caller's own documented fields are forwarded"
  );
});
...
test("the proxy forwards the caller's payload and credential upstream while withholding local routing metadata", async () => {
  ...
  const proxyPort = 18_700 + (process.pid % 500);
  const child = spawn(process.execPath, [PROXY], {
    env: {
      ...process.env,
      MINIMAX_PROXY_HOST: "127.0.0.1",
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${proxyPort}/v1/responses`, {
      ...
      headers: {
        ...
        [FORWARDED_REQUEST_HEADERS[0]!]: JSON.stringify({
          workspaces: { "/Users/someone/private-repo": {} }
        }),
    ...
    assert.equal(
      (JSON.parse(upstreamRequest!.body) as { model: string }).model,
      "MiniMax-M3",
      "the parent's own payload is forwarded unchanged"
    );
    assert.equal(
      upstreamRequest!.headers[FORWARDED_REQUEST_HEADERS[0]!],
      undefined,
      "workspace paths must not leave the machine"
    );
    assert.equal(
      upstreamRequest!.headers[AGENT_ROLE_HEADER],
      undefined,
      "local routing classification must not leave the machine"
    );
...
test("the proxy forwards namespace tools unchanged instead of flattening them", async () => {
  ...
  const upstream = createServer((request, response) => {
    ...
    request.on("data", (chunk: Buffer) => chunks.push(chunk));
    request.on("end", () => {
      upstreamRequestBody = JSON.parse(
...
test("the proxy preserves web search and fetch tools instead of treating them as code-mode tools", async () => {
  ...
  const upstreamPort = (upstream.address() as { port: number }).port;
  const proxyPort = 18_800 + (process.pid % 500);
  const child = spawn(process.execPath, [PROXY], {
    env: {
      ...process.env,
      MINIMAX_PROXY_HOST: "127.0.0.1",
...
test("exec called as though it were exec_command becomes a runnable script", () => {
  ...
  assert.deepEqual(call, {
    cmd: "wc -l < agents/prompts/leaf.md",
    workdir: "/Users/henrykirk/AutoDev"
  });
  assert.doesNotThrow(
...
test("a model that already understood code mode is passed through", () => {
  assert.equal(
    freeformInputFromArguments('{"input":"text(1 + 1)"}'),
    "text(1 + 1)"
  );
...
test("a response declaring no freeform tools is returned unchanged", () => {
  const body = {
    response: {
      output: [
        { type: "function_call", id: "fc_1", name: "exec", arguments: "{}" }
      ]
    }
  };
...
test("normalising item ids upstream leaves everything MiniMax relies on intact", async () => {
  ...
  try {
    ...
    const payload = upstreamPayload(
      routing.routeForModel("MiniMax-M3") as unknown as Parameters<
    ...
    assert.deepEqual(
      sent.map((item) => item.call_id),
      history.map((item) => item.call_id)
    );
    ...
    assert.equal(
      upstreamRequestBody!.tools[0]!.name,
...
test("an unrecognised exec call becomes a failing script that tells the model what exec expects", () => {
  for (const [argumentsText, shape] of [
    ...
    const message = JSON.parse(
      script.slice("throw new Error(".length, script.lastIndexOf(")"))
    ) as string;
    ...
    assert.equal(
      script.includes("do-not-echo-this-value"),
      false,
      "argument values must never be echoed"
    );
    assert.doesNotThrow(
      () => new Function(`return (async () => {\n${script}\n});`)
    );
...
test("a recognised exec call is still translated, not replaced with feedback", () => {
  ...
  const [item] = (
    coerceResponseBody(body, new Set(["exec"])) as {
...
test("a streamed unrecognised exec call reaches Codex as a custom tool call carrying the feedback script", async () => {
    ...
    .map(
      (line) =>
        JSON.parse(line.slice(6)) as {
          ...
          input?: string;
          item?: { type: string };
          response?: { output: Array<{ input: string }> };
        }
    );
  ...
  assert.equal(
    received.find((event) => event.type === "response.output_item.done")!.item!
      .type,
    "custom_tool_call"
  );
  assert.equal(
    received.find((event) => event.type === "response.completed")!.response!
      .output[0]!.input,
    feedback
  );
  const headers = streamed
    .split("\n")
