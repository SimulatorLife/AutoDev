...
const contractPath = join(repositoryRoot, 'tests/fixtures/otel/autodev-attributes-emission-contract.json');
const existingFixturePath = join(repositoryRoot, 'tests/fixtures/otel/collector-forwarded-otlp.json');
const expectedKeys = ['autodev.role', 'autodev.workspace', 'autodev.provider', 'autodev.model', 'autodev.spawn.mechanism', 'autodev.skill', 'autodev.mcp.server'] as const;
const resourceKeys = new Set(['autodev.role', 'autodev.workspace', 'autodev.provider', 'autodev.model']);
const eventKeys = new Set(['autodev.spawn.mechanism', 'autodev.skill', 'autodev.mcp.server']);
...
function attributes(value: Json | undefined): Attribute[] { return array(value).map((entry) => object(entry)); }
...
function attributeLocations(value: Json, path = ''): AttributeLocation[] {
  if (Array.isArray(value)) return value.flatMap((child, index) => attributeLocations(child, `${path}[${index}]`));
...
function signalLocations(container: JsonObject, signal: string): Map<string, Attribute[]> {
...
function isResourceLocation(location: string): boolean {
  return location.includes('.resource.') || location.startsWith('resource.') || location.endsWith('.resource.attributes')
    || (location.includes('resourceLogs[') && location.includes('resource.attributes'))
    || (location.includes('resourceSpans[') && location.includes('resource.attributes'))
    || (location.includes('resourceMetrics[') && location.includes('resource.attributes'));
}
function stringified(value: Json | undefined): string { return JSON.stringify(value) ?? String(value); }
...
test('emission contract metadata and ordered cross-references match the schema', () => {
  assert.equal(contract.contract_version, 'autodev-otel-attributes-emission-v1');
  ...
  const schemaKeys = attributes(schema.attributes).map((entry) => String(entry.key));
  assert.deepEqual(crossReference.ordered_keys, expectedKeys);
  assert.deepEqual(crossReference.ordered_keys, schemaKeys);
  assert.deepEqual(attributes(crossReference.mappings).map((entry) => String(entry.key)), expectedKeys);
});
...
test('emission scope and signal cross-references match the canonical schema', () => {
  ...
  const schemaScope = Object.fromEntries(schemaEntries.map((entry) => [String(entry.key), entry.scope]));
  const schemaSignals = Object.fromEntries(schemaEntries.map((entry) => [String(entry.key), entry.signals]));
  ...
  for (let index = 0; index < expectedKeys.length; index += 1) {
    assert.equal(mappings[index]?.scope, schemaScope[expectedKeys[index]!] );
    assert.deepEqual(mappings[index]?.signals, schemaSignals[expectedKeys[index]!] );
  }
});
...
test('before and after mappings preserve every signal, location, and existing value', () => {
  for (const signal of ['logs', 'traces', 'metrics']) {
    ...
    assert.deepEqual([...afterLocations.keys()].sort(), [...beforeLocations.keys()].sort(), signal);
    for (const [location, beforeAttributes] of beforeLocations) {
      ...
      for (const [key, value] of attributeMap(beforeAttributes)) {
        ...
        assert.deepEqual(afterMap.get(key), value, `${signal}/${location}/${key}`);
      }
    }
  }
});
  ...
  for (const signal of ['logs', 'traces', 'metrics']) {
    ...
    for (const [location, beforeAttributes] of beforeLocations) {
      ...
      for (const key of addedKeys(beforeAttributes, afterAttributes)) {
        assert.equal(key.startsWith('autodev.'), true, `${signal}/${location}/${key}`);
        assert.equal((expectedKeys as readonly string[]).includes(key), true, key);
        allAdded.add(key);
      }
    }
  }
...
test('resource and event AutoDev keys stay in their respective attribute scopes', () => {
  for (const signal of ['logs', 'traces', 'metrics']) for (const [location, entries] of signalLocations(after, signal)) {
    const map = attributeMap(entries);
    for (const key of resourceKeys) if (map.has(key)) assert.equal(isResourceLocation(location), true, `${signal}/${location}/${key}`);
    for (const key of eventKeys) if (map.has(key)) assert.equal(isResourceLocation(location), false, `${signal}/${location}/${key}`);
  }
});

test('unknown optional values produce no AutoDev keys', () => {
  for (const side of ['before', 'after'] as const) for (const signal of ['logs', 'traces', 'metrics']) {
    for (const [location, entries] of signalLocations(omission[side] as JsonObject, signal)) {
      const keys = [...attributeMap(entries).keys()].filter((key) => key === 'autodev' || key.startsWith('autodev.'));
      assert.deepEqual(keys, [], `${side}/${signal}/${location}`);
    }
  }
  for (const [location, entries] of signalLocations(after, 'traces')) if (location.includes('spans[3]')) {
    assert.deepEqual([...attributeMap(entries).keys()].filter((key) => key.startsWith('autodev.')), [], location);
  }
  for (const [location, entries] of signalLocations(after, 'logs')) if (location.includes('logRecords[6]')) {
    assert.deepEqual([...attributeMap(entries).keys()].filter((key) => key.startsWith('autodev.')), [], location);
  }
});
...
test('AutoDev attributes are categorical and never carry prompt content', () => {
  ...
  for (const signal of ['logs', 'traces', 'metrics']) for (const [location, entries] of signalLocations(after, signal)) {
    for (const [key, value] of attributeMap(entries)) if (key.startsWith('autodev.')) {
      assert.equal(stringified(value).includes(secret), false, `${signal}/${location}/${key}`);
      assert.ok(stringified(value).length <= 64, `${signal}/${location}/${key}`);
    }
  }
  for (const [location, entries] of signalLocations(after, 'logs')) if (location.includes('logRecords[1]')) {
    const map = attributeMap(entries);
    assert.deepEqual(map.get('event.name'), { stringValue: 'codex.user_prompt' });
    assert.equal(map.has('prompt_text'), true);
    assert.equal(map.has('prompt_length'), true);
    assert.deepEqual([...map.keys()].filter((key) => key.startsWith('autodev.')), [], location);
  }
});
...
test('runtime emission remains opt-in and the existing Collector fixture is undecorated', () => {
  ...
  for (const [location, entries] of attributeLocations(readJson(existingFixturePath))) {
    assert.deepEqual([...attributeMap(entries).keys()].filter((key) => key === 'autodev' || key.startsWith('autodev.')), [], location);
  }
});
