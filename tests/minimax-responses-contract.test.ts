...
const contract = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL(
        "fixtures/contracts/minimax-responses-contract.json",
        import.meta.url
      )
    ),
    "utf8"
...
assert.equal(
  contract.schema,
  "autodev-minimax-responses-contract-v2",
  "MiniMax boundary contract must match its schema tag"
);
...
describe("MiniMax boundary contract", () => {
  describe("namespace_tools_forwarded", () => {
    ...
    test("request tools reach MiniMax exactly as Codex (or the router) sent them", () => {
      assert.deepEqual(
        rewriteOutboundPayload(entry.request)?.tools,
        entry.request.tools
      );
    });
    ...
    test("MiniMax's native namespace on a tool call is part of the contract the adapter relies on", () => {
      assert.deepEqual(
        entry.upstreamResponse.output
          .filter((item: JsonObject) => item.type === "function_call")
          .map((item: JsonObject) => item.namespace),
    ...
    test("isWebResearchTool keeps Codex-native web research tools out of freeform coercion", () => {
      ...
      assert.equal(
        isWebResearchTool({ type: "function", name: "read_file" }),
        false
      );
    });
  });
  ...
  describe("freeform_coercion_non_streaming", () => {
    ...
    test("coerceResponseBody rewrites a freeform function_call into a custom_tool_call", () => {
      ...
      assert.match(
        ...
        new RegExp(
          String(entry.expected.inputSource.startsWith).replaceAll(
            /[.*+?^${}()|[\]\\]/g,
            String.raw`\$&`
          )
        )
      ...
      assert.match(
        String(rewritten.input),
        new RegExp(
          String(entry.expected.inputSource.mentions).replaceAll(
            /[.*+?^${}()|[\]\\]/g,
            String.raw`\$&`
          )
        )
  ...
  describe("freeform_unrecognised_feedback", () => {
    ...
    test("an unrecognisable freeform call becomes an explanatory failing script that never echoes argument values", () => {
      const coerced = coerceResponseBody(
        entry.upstreamResponse,
        new Set(["exec"])
      );
      assert.ok(coerced && typeof coerced === "object");
      ...
      assert.ok(
        input.startsWith(String(entry.expected.inputSource.startsWith))
      );
      const message = JSON.parse(
        input.slice(
          String(entry.expected.inputSource.startsWith).length,
          input.lastIndexOf(")")
        )
      );
