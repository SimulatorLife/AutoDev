...
type RecordedRow = { url?: string; headers?: Record<string, string>; body?: Record<string, unknown> };

async function withRecorder(options: Omit<Partial<RecorderOptions>, "record">, run: RecorderRun): Promise<void> {
...
const rows = async (record: string): Promise<RecordedRow[]> => (await readFile(record, "utf8")).split("\n").filter(Boolean).map((line) => JSON.parse(line) as RecordedRow);
...
test("replay mode streams scripted turns in order, repeats the last one, and records every follow-up", async () => {
  const turns: SseEvent[][] = [ [ { type: "response.created", response: { id: "r1" } } ], [ { type: "response.completed", response: { id: "r2" } } ] ];
...
test("the example turns reproduce MiniMax-M3's nested exec_command call, then finish", async () => {
  const fixture = JSON.parse(await readFile(join(SKILL_DIR, "examples", "exec-command.turns.json"), "utf8")) as { turns: Array<Array<{ type: string; item?: { type?: string; name?: string; id?: string; arguments?: string } }>> };
  const { turns } = fixture;
  assert.equal(turns.length, 2);
  const callEvent = turns[ 0 ]?.find((event) => event.type === "response.output_item.done" && event.item?.type === "function_call");
