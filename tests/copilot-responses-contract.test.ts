...
`;
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${proxyPort}/v1/responses`, {
      ...
      body: JSON.stringify({ model: "copilot", input: [{ role: "user", content: "contract task" }], cwd: REPO_ROOT, ...item.request }),
    });
    ...
    if (item.expected.directEventTypes) {
      ...
      try {
        ...
        if (previousContractCase === undefined) delete process.env.COPILOT_CONTRACT_CASE;
    ...
    assert.doesNotMatch(JSON.stringify(normalizedSse), /contract task/, `${name}: prompt content must not be in SSE`);
    assert.doesNotMatch(JSON.stringify(reported), /contract task/, `${name}: prompt content must not be in telemetry`);
    return scrub(sse);
  } finally {
    child.kill("SIGTERM");
