...
const UNSUPPORTED_SUBSCRIPTION_TRANSPORTS = new Map([
  ...
  [
    ...
    "Copilot model endpoint reached without the Copilot CLI"
  ],
  ["github_copilot/", "LiteLLM GitHub Copilot provider route"],
  [
    "cloudcode-pa.googleapis.com",
    "Antigravity/Gemini CLI OAuth backend reached without agy"
  ]
]);
...
function trackedRuntimeTexts(): RuntimeTexts {
  ...
  for (const path of listed.split("\n").filter(Boolean)) {
    ...
    try {
      ...
      texts[path] = path.endsWith(".ts") ? normalizedSource(text) : text;
    } catch {
      /* deleted or binary */
    }
  }
...
for (const [needle, reason] of UNSUPPORTED_SUBSCRIPTION_TRANSPORTS) {
  test(`no runtime file uses ${reason}`, () => {
    assert.deepEqual(
      Object.keys(texts).filter((path) => texts[path]?.includes(needle)),
      []
    );
  });
}
...
test("every Codex model provider targets a local adapter", () => {
  const config = parse(
    readFileSync(join(REPO_ROOT, "config/config.autodev.toml"), "utf8")
  ) as JsonRecord;
  const providers = (config.model_providers ?? {}) as JsonRecord;
  for (const [name, provider] of Object.entries(providers)) {
    assert.match(
      String((provider as JsonRecord).base_url ?? ""),
      /^http:\/\/127\.0\.0\.1:\d+\/v1$/,
      name
    );
  }
...
test("only reviewed runtime files reference the Claude subscription token", () => {
  const consumers = new Set(
    Object.keys(texts).filter((path) => texts[path]?.includes(CLAUDE_TOKEN))
  );
...
test("CI runs the pinned official Claude Code package", () => {
  const manifest = JSON.parse(
    readFileSync(join(REPO_ROOT, ".github/ci/provider-tools.json"), "utf8")
  ) as JsonRecord;
  assert.match(
