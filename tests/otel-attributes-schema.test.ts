...
function attributeEntries(value: Json | undefined): JsonObject[] {
...
const byKey = Object.fromEntries(
  attributes.map((entry) => [String(entry.key), entry])
);
...
test("AutoDev attributes have typed, scoped, optional, additive definitions", () => {
  for (const key of expectedKeys) {
    ...
    assert.equal(
      signals.every((signal) => validSignals.has(signal)),
      true
    );
  ...
  assert.deepEqual(
    Object.keys(byKey)
      .filter((key) => byKey[key]?.scope === "resource")
      .sort(),
    ["autodev.model", "autodev.provider", "autodev.role", "autodev.workspace"]
  );
  assert.deepEqual(
    Object.keys(byKey)
      .filter((key) => byKey[key]?.scope === "event")
      .sort(),
    ["autodev.mcp.server", "autodev.skill", "autodev.spawn.mechanism"]
  );
});

test("AutoDev attribute names do not collide with existing unprefixed keys", () => {
  assert.deepEqual(
    Object.keys(byKey).filter((key) => existingKeys.has(key)),
    []
  );
});
