...
import { awaitedCallIds, ToolCallOwnership } from '../../src/router/tool-call-ownership.ts';
...
test('the tool results ending the input name the calls a request is answering', () => {
  const payload = {
    input: [
      { type: 'custom_tool_call_output', call_id: 'call_old', output: 'settled earlier' },
  ...
  assert.deepEqual(awaitedCallIds({ input: [ { type: 'message', role: 'user', content: 'new turn' } ] }), []);
  // A notification Codex appends after the output keeps the results awaited.
  const notified = { input: [ ...payload.input, { type: 'message', role: 'user', content: '<subagent_notification>{}</subagent_notification>' } ] };
...
test('the provider that issued a call owns its result', () => {
  ...
  assert.equal(ownership.ownerFor({ input: [ { type: 'custom_tool_call_output', call_id: 'call_a', output: '' } ] }), 'claude');
  assert.equal(ownership.ownerFor({ input: [ { type: 'function_call_output', call_id: 'call_unknown', output: '' } ] }), null);
  assert.equal(ownership.ownerFor({ input: [ { type: 'message', role: 'user', content: 'hi' } ] }), null);
});
...
test('ownership is bounded, oldest calls first', () => {
  ...
  const answering = (callId: string) => ({ input: [ { type: 'custom_tool_call_output', call_id: callId, output: '' } ] });
...
test('call ids are collected from streamed items and from completed responses', () => {
  ...
  collectToolCallIds({ type: 'custom_tool_call', id: 'ctc_1', call_id: 'call_a', name: 'exec', input: '' }, ids);
  collectToolCallIds({ output: [ { type: 'message', id: 'msg_1', content: [] }, { type: 'function_call', id: 'fc_1', call_id: 'call_b', name: 'wait', arguments: '{}' } ] }, ids);
