...
test('orchestrator-active-subagents: orchestrator remains in subagent_wait while subagents are active', () => {
  ...
  try {
    ...
    const orchestratorModel = 'claude-opus-5';
    ...
    assert.equal(orchestratorProviderForSession(sessionKey), orchestratorProvider);
    ...
    assert.equal(agentActivity.getState(sessionKey), 'subagent_wait');
    assert.equal(agentActivity.getRecord(sessionKey).provider, orchestratorProvider);
    assert.equal(agentActivity.getRecord(sessionKey).role, ORCHESTRATOR_AGENT_ROLE);
...
test('orchestrator-active-subagents: bridge subagent usage keeps orchestrator session in subagent_wait', () => {
  ...
  try {
    ...
    closeBridgeSubagentUsage('req-bridge-parent\0child-1', { outcome: 'success' });
    assert.equal(agentActivity.getState(sessionKey), 'resumed');
  } finally {
    resetRouterTelemetry();
    agentActivity.reset();
