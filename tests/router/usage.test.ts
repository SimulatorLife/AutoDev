...
test("safeWorkspaceId and privacy helpers hash local paths and sanitize metric labels", () => {
  ...
  const winId = safeWorkspaceId(String.raw`C:\Users\dev\project`);
  ...
  assert.equal(
    safeAgentIdentity("/home/user/custom-agent"),
    `agent_${safeWorkspaceId("/home/user/custom-agent").slice(3)}`
  );
  ...
  assert.match(
    safePrivacyWorkspace("/Users/someone/repo"),
    /^ws_[0-9a-f]{12}$/
  );
...
test("extractWorkspaceIdWithAmbiguity extracts workspace IDs and detects ambiguity", () => {
  assert.deepEqual(extractWorkspaceIdWithAmbiguity(null), {
    id: null,
    ambiguous: false
  });
  assert.deepEqual(extractWorkspaceIdWithAmbiguity({}), {
    id: null,
    ambiguous: false
  });
  assert.deepEqual(extractWorkspaceIdWithAmbiguity({ workspace_id: "ws-1" }), {
    id: "ws-1",
    ambiguous: false
  });
  assert.deepEqual(
    extractWorkspaceIdWithAmbiguity({ "workspace.id": "ws-2" }),
    { id: "ws-2", ambiguous: false }
  );
  assert.deepEqual(extractWorkspaceIdWithAmbiguity({ workspaceId: "ws-3" }), {
    id: "ws-3",
    ambiguous: false
  });
...
test("readNamedAttribute, toolServerAttribute, and toolKey extract tool attributes", () => {
  ...
  assert.equal(
    readNamedAttribute(attrs, "fallback", "nonexistent", "tool_name"),
    "read_file"
  );
  assert.equal(
    readNamedAttribute(attrs, "fallback", "nonexistent"),
    "fallback"
  );
...
test("Workspace registry registers, handles duplicates, and fails on conflict", () => {
  ...
  assert.equal(
    tracker.workspaceIdRegistry.get(safeWorkspaceId("ws-1")),
    "RepoA"
  );
...
test("Attribution diagnostics tracks counts, reason breakdowns, and caps unknown workspace IDs", () => {
  ...
  assert.equal(
    cappedStatus.unknownWorkspaceIds.length,
    MAX_UNKNOWN_WORKSPACE_IDS
  );
  assert.equal(cappedStatus.unknownWorkspaceIds.includes("ws-bad"), false);
  assert.equal(
    cappedStatus.unknownWorkspaceIds.includes("ws-unknown-119"),
    true
  );
  ...
  assert.equal(
    tracker.attributionDiagnosticsStatus().unknownWorkspaceIds.length,
    0
  );
});
...
test("Workspace bucket management and formatting", () => {
  ...
  const tool = tracker.workspaceToolBucket(wsBucket, {
    tool: "grep",
    source: "builtin",
    server: "search"
  });
...
test("recordUsageEvent updates counters across phase transitions", () => {
  ...
  assert.equal(
    tracker.usageTelemetry.totals.lastUsedAt,
    "2026-09-16T12:00:00Z"
  );
  assert.equal(tracker.usageTelemetry.byRole.explorer!.attempts, 1);
  assert.equal(
    tracker.usageTelemetry.byModel["anthropic/claude-3-5-sonnet"]!.attempts,
    1
  );
...
test("projectLiveAgents and usageStatus reflect live agent activity", () => {
  const activityTracker = createAgentActivityTracker({
    ttlMs: 60_000,
    now: () => 1000
  });
...
test("usagePersistenceSnapshot and restoreUsagePersistenceSnapshot preserve full telemetry state", () => {
  ...
  assert.equal(
    cleanTracker.workspaceIdRegistry.get(safeWorkspaceId("ws-pers")),
    "RepoPersistence"
  );

  // Verify unsupported schema versions fail closed (do not alter state)
  const emptyStateTracker = new UsageTracker();
  emptyStateTracker.restoreUsagePersistenceSnapshot({
    schemaVersion: 6,
    totals: { attempts: 999 }
  });
