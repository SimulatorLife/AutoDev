...
test("SubagentRegistry records spawns and maintains ring buffer and aggregations", () => {
  ...
  assert.equal(
    registry.recordSubagentSpawn({ mechanism: "invalid", count: 1 }),
    null
  );
  // Zero count returns null
  assert.equal(
    registry.recordSubagentSpawn({ mechanism: "router_alias", count: 0 }),
    null
  );

  registry.recordSubagentSpawn({
...
test("SubagentRegistry tracks orchestrator sessions and workspace metadata ignoring process fallback", () => {
  ...
  assert.equal(
    registry.orchestratorProviderForSession(PROCESS_FALLBACK_SESSION_KEY),
    null
  );
  assert.equal(
    registry.orchestratorProviderForSession("unknown-session"),
    null
  );

  registry.rememberWorkspaceMetadata("session-1", "/path/to/repo");
  assert.ok(
    registry.getWorkspaceMetadata("session-1")?.includes("/path/to/repo")
  );

  registry.rememberWorkspaceMetadata(
    PROCESS_FALLBACK_SESSION_KEY,
    "/path/to/repo"
  );
  assert.equal(
    registry.getWorkspaceMetadata(PROCESS_FALLBACK_SESSION_KEY),
    null
  );
});
...
test("SubagentRegistry tracks bridge request and session contexts", () => {
  ...
  registry.noteBridgeRequest("req-100", {
    activitySubject: `req:${"req-100"}`,
    provider: "claude",
    model: "sonnet",
    role: "worker"
  });
  assert.deepEqual(registry.getBridgeRequestContext("req-100"), {
    activitySubject: "req:req-100",
    provider: "claude",
    model: "sonnet",
    role: "worker"
  ...
  registry.noteBridgeSession("sess-100", {
    activitySubject: "sess-100",
    requestId: "req-100",
    provider: "claude",
    model: "sonnet"
  });
...
test("capabilities and telemetry headers resolve properly", () => {
  const mockExecutionContract = {
    ...
    roles: {
      ...
      explorer: {
        mcp: ["server-2"],
        skills: ["skill-2"],
        webResearch: { search: true, fetch: false }
      }
    }
  };
  ...
  assert.deepEqual(
    subagentSpawnToolsFor("antigravity", mockExecutionContract),
    ["invoke_subagent"]
  );
  assert.equal(
    providerCapabilities("minimax", mockExecutionContract).subagentSpawn,
    false
  );
  assert.equal(
    providerCapabilities("copilot", mockExecutionContract).subagentSpawn,
    true
  );
  assert.equal(
    providerCapabilities("missing", mockExecutionContract).subagentSpawn,
    false
  );
  assert.equal(
  ...
  assert.deepEqual(mcpContractForRole("orchestrator", mockExecutionContract), [
    "server-1"
  ]);
  assert.deepEqual(mcpContractForRole("explorer", mockExecutionContract), [
    "server-2"
  ]);

  const roleReqs = roleCapabilityRequirements(
    "explorer",
  ...
  assert.deepEqual(
    bridgeTelemetryHeaders({ provider: "codex" }, "req-123", {
  ...
  assert.equal(
    headers["x-autodev-agent-events-url"],
    "http://127.0.0.1:4100/v1/agent-events"
  );
...
test("convenience functions delegate to default SubagentRegistry", async () => {
  ...
  noteBridgeRequest("req-def", {
    activitySubject: `req:${"req-def"}`,
    provider: "antigravity",
    model: "flash"
  });
  assert.deepEqual(getBridgeRequestContext("req-def"), {
    activitySubject: "req:req-def",
    provider: "antigravity",
    model: "flash"
  });
...
test("synthetic bridge parent activity settles with the parent outcome", () => {
  ...
  registry.openBridgeSubagentUsage({
    requestId: "parent-failure",
    context,
    role: "worker",
    childId: "child-1"
  });
  ...
  assert.equal(
    registry.closeBridgeSubagentUsage("parent-failure\0child-1", {
      outcome: "success"
    }),
    true
  );
