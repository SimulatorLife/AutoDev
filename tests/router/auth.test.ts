...
test("resolveRouterAuthToken reads CODEX_ROUTER_AUTH_TOKEN from environment", () => {
  ...
  assert.equal(
    resolveRouterAuthToken({ CODEX_ROUTER_AUTH_TOKEN: "test-secret-token" }),
    "test-secret-token"
  );
});
...
test("setRouterAuthTokenForTests dynamically updates configured token and status", () => {
  try {
    ...
    assert.equal(
      routerAuthorizationValid({
        headers: { authorization: "Bearer dynamic-test-token" }
      }),
      true
    );
    assert.equal(
      routerAuthorizationValid({ headers: { authorization: "Bearer wrong" } }),
      false
    );
