...
test('classifyProviderFailure accurately classifies status codes and error bodies', () => {
  ...
  assert.equal(classifyProviderFailure(503, 'temporarily unavailable'), 'unavailable');
  ...
  assert.equal(classifyProviderFailure(404, 'unknown model gpt-9'), 'invalid_model');
  assert.equal(classifyProviderFailure(500, 'internal server error'), 'upstream_error');
  assert.equal(classifyProviderFailure(400, 'malformed request body'), 'request_error');
});
