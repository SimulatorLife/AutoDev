...
const contract: ContractFixture = JSON.parse(
  readFileSync(
    new URL(
      "fixtures/contracts/agent-reconciliation-contract.json",
      import.meta.url
    ),
    "utf8"
  )
...
assert.equal(
  contract.schema,
  "autodev-agent-reconciliation-contract-v1",
  "agent reconciliation contract must match its schema tag"
);
assert.equal(
  contract.statusAgentsSchema,
  "autodev-agent-status-v1",
  "the projected status.agents schema tag is frozen"
);
...
function evalOps(
  name: string,
  operations: ContractOp[],
  ctx: { status: Record<string, unknown> }
): void {
  for (const op of operations) {
    switch (op.op) {
      ...
      case "status_no_key": {
        assert.equal(
          Object.hasOwn(ctx.status, op.key!),
          false,
          `${name}: ${op.key} must not appear on status.agents`
        );
        break;
      }
      case "status_shape": {
        ...
        assert.deepEqual(
          keys,
          [...(op.expected as string[])].sort(),
          `${name}: shape${op.field ? ` of ${op.field}` : ""}`
        );
        break;
      }
      default: {
  ...
  try {
    for (const op of scenario.operations ?? []) {
      switch (op.op) {
        ...
        case "beginSlot": {
          slotSequence += 1;
          const subject =
            op.subject ?? `subagent_slot:${op.sessionKey}:${slotSequence}`;
      ...
      [
        {
          op: "status_field",
          field: "schema",
          expected: "autodev-agent-status-v1"
        }
      ],
      { status }
    );
    evalOps(
      ...
      [
        {
          op: "status_field",
          field: "reconciledWithConcurrency",
          expected: true
        }
      ],
      { status }
...
describe("agent reconciliation contract: constants and module surface", () => {
  ...
  test("agentsStatus(at) and concurrencyStatus(at) share the same `at` for slot reconciliation", () => {
    ...
    assert.equal(
      slotVsAgent.activeAdmissionSessions,
      concurrency.activeSessions
    );
    assert.equal(
      slotVsAgent.processFallbackActiveThreads,
      concurrency.processFallbackActiveThreads
    );
  });

  test("AGENT_ACTIVITY_KINDS exposes only session and bridge_subagent", () => {
    assert.deepEqual([...AGENT_ACTIVITY_KINDS].sort(), [
      "bridge_subagent",
      "session"
    ]);
  });
