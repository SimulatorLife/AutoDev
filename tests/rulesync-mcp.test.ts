...
const portableConfig = join(repositoryRoot, 'scripts/codex/config.autodev.toml');
const packageJson = join(repositoryRoot, 'package.json');
const rulesync = join(repositoryRoot, 'node_modules/.bin/rulesync');
const launchCommand = 'exec "${CODEX_HOME:-$HOME/.codex}/hooks/run-autodev-mcp.sh" ';
...
function source(): JsonObject { return readJson(mcpSource); }
...
function declared(target: string): Record<string, McpServer> {
  ...
  const targetConfig = targetSection && typeof targetSection === 'object' && !Array.isArray(targetSection)
...
function runRulesync(args: readonly string[], home?: string): RunResult {
  return spawnSync(rulesync, ['generate', '--input-roots', sourceRoot, '--features', 'mcp', ...args, '--silent'], {
    cwd: repositoryRoot,
    encoding: 'utf8',
    timeout: 60_000,
...
function generateUserLevel(home: string, ...extra: string[]): RunResult {
  return runRulesync(['--global', '--targets', Object.keys(userLevelFiles).join(','), ...extra], home);
}
function readMcpFile(path: string): Record<string, McpServer> { return serverMap(readJson(path).mcpServers); }

const temporaryRoot = mkdtempSync(join(tmpdir(), 'autodev-rulesync-mcp-'));
const codexRoot = join(temporaryRoot, 'codex');
const home = join(temporaryRoot, 'home');
mkdirSync(home, { recursive: true });
writeFileSync(join(home, '.claude.json'), JSON.stringify({ numStartups: 3, mcpServers: { stale: { command: 'stale' } } }));
const portableBefore = readFileSync(portableConfig);
for (const result of [runRulesync(['--targets', 'codexcli', '--output-roots', codexRoot]), generateUserLevel(home)]) {
  assert.equal(result.status, 0, `Rulesync MCP generation failed:\n${result.stdout}\n${result.stderr}`);
}
...
test('portable Codex config declares no MCP servers', () => {
  const config = parse(readFileSync(portableConfig, 'utf8')) as Record<string, unknown>;
...
test('source declares each launch definition consistently', () => {
  ...
  assert.deepEqual(Object.keys(document).sort(), ['$schema', 'antigravity-cli', 'codexcli', 'copilotcli', 'mcpServers'].sort());
  assert.deepEqual(Object.keys(shared).sort(), ['cocoindex-code', 'lsp', 'openaiDeveloperDocs']);
  for (const target of ['codexcli', 'claudecode', 'copilotcli', 'antigravity-cli']) {
    const targetServers = declared(target);
    assert.deepEqual([...forbiddenServers].filter((name) => Object.hasOwn(targetServers, name)), []);
    assert.equal(Object.hasOwn(targetServers, 'autodev_spawn'), target === 'antigravity-cli');
    for (const [name, server] of Object.entries(targetServers)) {
      if (name === 'autodev_spawn') {
        ...
        assert.equal(String(server.args[1]).endsWith('/src/mcp/spawn-shim.ts"'), true);
      } else if (Object.hasOwn(server, 'command')) {
        assert.equal(server.command, 'bash');
        assert.deepEqual(server.args, ['-lc', `${launchCommand}${name}`]);
      }
      if (Object.hasOwn(shared, name)) {
        for (const key of ['command', 'args', 'url']) assert.deepEqual(server[key], shared[name]?.[key]);
      }
    }
  }
});
...
test('Codex projection matches the Codex declaration without mutating the portable source', () => {
  ...
  const generated = serverMap((parse(readFileSync(join(codexRoot, '.codex/config.toml'), 'utf8')) as Record<string, unknown>).mcp_servers);
  ...
  for (const [name, server] of Object.entries(expected)) {
    ...
    for (const key of ['command', 'args', 'url', 'default_tools_approval_mode']) if (Object.hasOwn(server, key)) projected[key] = server[key];
...
test('user-level files list exactly each target declaration and preserve non-MCP keys', () => {
  for (const [target, relativePath] of Object.entries(userLevelFiles)) {
    ...
    assert.deepEqual(Object.keys(servers).sort(), Object.keys(expected).sort(), target);
    ...
    for (const [name, server] of Object.entries(expected)) {
      ...
      assert.equal(Object.hasOwn(projected, 'default_tools_approval_mode'), false);
    }
  }
...
test('Rulesync check catches edited and extra user-level servers', () => {
  ...
  for (const [label, damage] of [['edited', editAntigravity], ['extra', addCopilotServer]] as const) {
