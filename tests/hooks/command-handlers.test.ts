...
test("session-start delegates the typed ensure runner and preserves a healthy status", async () =>
  withCodexHome(async (home) => {
    const seen: { deps: RouterEnsureDeps; options: RouterEnsureOptions }[] = [];
    const runner = {
      async runRouterEnsure(
