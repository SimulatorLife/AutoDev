...
test('isCccInvocation identifies direct and compound ccc CLI invocations', () => {
  ...
  for (const cmd of blocked) {
    assert.equal(isCccInvocation(cmd), true, `expected "${cmd}" to be identified as ccc invocation`);
  }
});
...
test('isCccInvocation ignores non-ccc commands and harmless substrings', () => {
  ...
  for (const cmd of allowed) {
    assert.equal(isCccInvocation(cmd), false, `expected "${cmd}" NOT to be identified as ccc invocation`);
  }
});
