...
function hooksAt(value: unknown): Record<string, Hook[]> {
  ...
  for (const [key, entries] of Object.entries(asObject(value))) result[key] = (entries as unknown[]).map(asObject) as Hook[];
  return result;
}
function commands(config: JsonObject): string[] {
  return Object.values(hooksAt(config.hooks)).flatMap((entries) => entries.filter((hook) => hook.type === 'command').map((hook) => String(hook.command)));
}
function groupedCommands(events: JsonObject): string[] {
  return Object.values(hooksAt(events)).flatMap((groups) => groups.flatMap((group) => {
    const hooks = group.hooks;
    return Array.isArray(hooks) ? hooks.map(asObject).filter((hook) => hook.type === 'command').map((hook) => String(hook.command)) : [];
  }));
}
function generate(target: string, outputRoot: string): void {
  const result = spawnSync('pnpm', ['exec', 'rulesync', 'generate', '--input-roots', sourceRoot, '--targets', target, '--features', 'hooks', '--output-roots', outputRoot, '--delete', '--silent'], { cwd: repositoryRoot, encoding: 'utf8', timeout: 60_000 });
  assert.equal(result.status, 0, `Rulesync ${target} hook generation failed:\nSTDOUT=${result.stdout}\nSTDERR=${result.stderr}`);
}
...
const outputs = Object.fromEntries(targets.map((target) => [target, join(temporaryRoot, target)])) as Record<(typeof targets)[number], string>;
...
function document(target: (typeof targets)[number]): JsonObject { return readJson(join(outputs[target], hookPaths[target])); }
...
test('Rulesync hook source is the canonical four-section command subset', () => {
  ...
  assert.deepEqual(Object.keys(asObject(source.hooks)).sort(), ['beforeSubmitPrompt', 'preToolUse', 'sessionStart', 'subagentStart']);
...
test('portable config keeps hooks in Rulesync and install uses typed materialization', () => {
  const portable = parse(readFileSync(portableConfigPath, 'utf8')) as Record<string, unknown>;
...
test('target projections preserve their documented command losses', () => {
  const projected = {
    ...
    claudecode: new Set(groupedCommands(asObject(document('claudecode').hooks))),
    copilot: new Set(commands(document('copilot'))),
    'antigravity-cli': new Set(groupedCommands(asObject(document('antigravity-cli').rulesync))),
  };
...
test('generation writes one hook file per target and leaves the portable source unchanged', () => {
  ...
  for (const target of targets) assert.deepEqual(generatedFiles(outputs[target]), [hookPaths[target]], target);
});

test('configured Rulesync generation writes hooks alongside skills', () => {
  const output = mkdtempSync(join(temporaryRoot, 'configured-'));
  try {
    const result = spawnSync('pnpm', ['exec', 'rulesync', 'generate', '--config', 'rulesync.jsonc', '--output-roots', output, '--delete', '--silent'], { cwd: repositoryRoot, encoding: 'utf8', timeout: 60_000 });
    assert.equal(result.status, 0, result.stderr);
    for (const target of targets) assert.equal(existsSync(join(output, hookPaths[target])), true, target);
...
test('Codex and Claude carry every hook with matcher and status metadata', () => {
  ...
  for (const target of ['codexcli', 'claudecode'] as const) {
    ...
    assert.deepEqual(Object.keys(hooks).sort(), ['PreToolUse', 'SessionStart', 'SubagentStart', 'UserPromptSubmit'].sort());
    assert.deepEqual(groupedCommands(hooks), sourceCommands);
    const targetPromptHooks = Array.isArray(hooks.UserPromptSubmit?.[0]?.hooks) ? hooks.UserPromptSubmit[0].hooks as unknown[] : [];
    assert.equal(asObject(targetPromptHooks[0]).statusMessage, source.beforeSubmitPrompt?.[0]?.statusMessage);
    assert.equal(hooks.PreToolUse?.[0]?.matcher, source.preToolUse?.[0]?.matcher);
  }
});

test('Copilot and Antigravity retain their documented parity limits', () => {
  const copilot = document('copilot');
  assert.deepEqual(Object.keys(asObject(copilot.hooks)), ['userPromptSubmitted']);
