...
async function startTelemetryServer(): Promise<{
  ...
  const address = server.address();
  if (!address || typeof address === "string")
    throw new Error("telemetry server did not expose a TCP address");
  ...
  try {
    ...
    if (item.expected.tool)
      assert.ok(
        reported.some(
          (event) =>
            event.type === "tool_requested" && event.tool === item.expected.tool
        ),
        `${name}: requested tool telemetry`
      );
    ...
    if (item.expected.directRead !== undefined) {
      ...
      assert.equal(
        update.tool_name.toLowerCase() === "read_file",
        item.expected.directRead,
        `${name}: direct-vs-shell read shape`
      );
      assert.equal(
        skillPath,
        `${REPO_ROOT}/.rulesync/skills/ccc/SKILL.md`,
        `${name}: extracted skill path`
      );
      assert.equal(
        matchSkillReadPath(skillPath),
        "ccc",
    ...
    assert.doesNotMatch(
      JSON.stringify(sse),
      new RegExp(item.expected.privacyToken),
...
test("Antigravity Responses contract fixture is exercised through the offline proxy boundary", async () => {
  ...
  try {
    for (const [name, item] of Object.entries(contract.cases))
      await runCase(name, item, telemetry);
  } finally {
