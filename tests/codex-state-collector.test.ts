...
// a single row, both of which the test scenarios drive explicitly.
type TestTables = Record<string, SqliteRow[]>;
interface InMemoryDatabase {
  tables: TestTables;
  tableInfo(name: string): SqliteRow[];
  records(name: string): SqliteRow[];
  addThread(row: SqliteRow): void;
  addProject(row: SqliteRow): void;
  addEdge(row: SqliteRow): void;
}
...
function fakeSqliteBinding(db: InMemoryDatabase): SqliteBinding {
  return {
    available: true,
