...
test('CLI dispatches router, provider, hook, and install through typed backends', () => {
  ...
  const result = runMain(['router', 'run'], {
    router: {
      run: () => { calls.push('router run'); return 11; },
      ensure: () => { calls.push('router ensure'); return 12; },
...
test('router status uses its typed status result', () => {
  ...
  try {
    assert.equal(runMain(['router', 'status'], { router: {
      run: () => 0,
      ensure: () => 0,
      status: () => ({ state: 'running', endpoint: 'http://127.0.0.1:4100', pid: 42 }),
    } }), 0);
  } finally {
    console.log = originalLog;
  }
  assert.deepEqual(JSON.parse(output[0] ?? ''), { state: 'running', endpoint: 'http://127.0.0.1:4100', pid: 42 });
});
