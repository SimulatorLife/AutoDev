...
test("a non-spawn tool never marks the turn as delegating", () => {
  ...
  const result = updateDelegationState(
    state,
    { step_index: 1, state: "ACTIVE", tool_name: "run_command" },
    isSpawnTool
  );
...
test("entering a delegator step records the tool and step index", () => {
  ...
  const result = updateDelegationState(
    state,
    { step_index: 3, state: "ACTIVE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
...
test("a second delegator overwrites the first when the first has not closed", () => {
  ...
  const result = updateDelegationState(
    state,
    { step_index: 4, state: "ACTIVE", tool_name: "manage_subagents" },
    isSpawnTool
  );
  assert.equal(result.kind, "entered");
...
test("DONE on the same step_index clears the active tool", () => {
  ...
  const result = updateDelegationState(
    state,
    { step_index: 3, state: "DONE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
...
test("a non-spawn tool event also clears the tracker for the same step", () => {
  ...
  const result = updateDelegationState(
    state,
    { step_index: 3, state: "DONE", tool_name: "agent_response" },
    isSpawnTool
  );
  assert.equal(result.kind, "exited");
  assert.equal(state.activeTool, null);
...
test("ERROR / FAILED / CANCELLED on the active step also clears the tracker", () => {
  for (const terminal of ["ERROR", "FAILED", "CANCELLED"]) {
    ...
    updateDelegationState(
      state,
      { step_index: 5, state: "ACTIVE", tool_name: "invoke_subagent" },
      isSpawnTool
    );
    const result = updateDelegationState(
      state,
      { step_index: 5, state: terminal, tool_name: "invoke_subagent" },
      isSpawnTool
    );
    ...
    assert.equal(
      state.activeTool,
      null,
      `terminal state ${terminal} must clear`
    );
  }
});
...
test("DONE on a different step_index is treated as unchanged", () => {
  ...
  updateDelegationState(
    state,
    { step_index: 7, state: "ACTIVE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
  const result = updateDelegationState(
    state,
    { step_index: 6, state: "DONE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
...
test("a step_update without an index still tracks but cannot be paired by id", () => {
  ...
  updateDelegationState(
    state,
    { state: "ACTIVE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
  assert.equal(state.activeTool, "invoke_subagent");
  const result = updateDelegationState(
    state,
    { state: "DONE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
  assert.equal(result.kind, "exited");
});
...
test("updateDelegationState tolerates a null state object", () => {
  const result = updateDelegationState(
    null as unknown as DelegationState,
    { step_index: 1, state: "ACTIVE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
  assert.deepEqual(result, { kind: "unchanged" });
});
...
test("updateDelegationState tolerates a missing isSpawnTool callback", () => {
  ...
  const result = updateDelegationState(
    state,
    { step_index: 1, state: "ACTIVE", tool_name: "invoke_subagent" },
    undefined as unknown as (name: string) => boolean
  );
  assert.deepEqual(result, { kind: "unchanged" });
  assert.equal(state.activeTool, null);
});
...
test("isDelegationActive stays true after ACTIVE -> DONE while children are still pending", () => {
  ...
  assert.equal(
    ...
    true,
    "an active dispatch step is delegation in flight"
  );
  ...
  const transition = updateDelegationState(
    state,
    { step_index: 3, state: "DONE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
  ...
  assert.equal(
    state.activeTool,
    null,
    "the dispatch step itself is no longer active"
  );
  assert.equal(
    isDelegationActive(state),
    true,
    "a pending child keeps delegation active even with no active step"
  );
...
test("decideCloseOnDelegation does not kill while pending children exist, even with no active step", () => {
  ...
  assert.equal(
    decision.kill,
    false,
    "children invoke_subagent dispatched must not be stranded by a kill"
  );
  assert.equal(decision.reason, "client_disconnected");
  assert.equal(
    decision.tool,
    null,
    "no dispatch step is active, so there is no tool to name"
  );
...
test("the heartbeat's own gate (isDelegationActive) stays open across a dispatch DONE with open children", () => {
  ...
  assert.equal(
    heartbeatShouldTick(),
    true,
    "ticks while the dispatch step is active"
  );

  updateDelegationState(
    state,
    { step_index: 1, state: "DONE", tool_name: "invoke_subagent" },
    isSpawnTool
  );
  assert.equal(
    heartbeatShouldTick(),
    true,
    "keeps ticking after DONE because a child is still pending"
  );
  ...
  assert.equal(
    heartbeatShouldTick(),
    false,
    "stops once the last pending child has closed"
  );
});
...
test("an end-to-end dispatch: spawn tracker + delegation state agree children outlive the DONE step", () => {
  ...
  const tracker = createSpawnTracker(
    reporter as unknown as Parameters<typeof createSpawnTracker>[0]
  );
  const state = freshState();
  ...
  const active = {
    ...
    tool_info: { args: { Subagents: [{ TypeName: "explorer" }] } }
  };
  ...
  assert.equal(
    decideCloseOnDelegation(state).kill,
    false,
    "a disconnect here must not strand the open child"
  );
  ...
  assert.equal(
    ...
    1,
    "the pending child's telemetry closes exactly once"
  );
  assert.equal(
    decideCloseOnDelegation(state).kill,
    true,
    "nothing is pending any more, so an ordinary disconnect kills agy"
  );
  ...
  assert.equal(
    reporter.results.length,
    1,
    "closing an already-closed spawn is a no-op"
  );
});
...
test("delegation tracking still recognizes agy's own spawn tool when the router sent no reporter", () => {
  ...
  assert.equal(
    ...
    1,
    "agy's own spawn tool is still recognized without a reporter"
  );
...
test("decideCloseOnDelegation does not kill while active commands or waits are running", () => {
  const cmdState: DelegationState = {
    activeTool: null,
    activeStep: null,
    activatedAt: 0,
    pendingChildren: 0,
  ...
  const waitState: DelegationState = {
    activeTool: null,
    activeStep: null,
    activatedAt: 0,
    pendingChildren: 0,
