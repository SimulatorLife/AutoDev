...
- **Legacy seed retirement:** `scripts/codex/config.toml` is no longer tracked or authoritative. An older installation with a valid symlink target is migrated once into a regular composed file at `$CODEX_HOME/config.toml`; a broken legacy symlink fails closed rather than discarding machine-local state.
...
`bash scripts/codex/install-codex-integration.sh --check` must pass afterward.
