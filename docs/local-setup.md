...
- **Authoritative portable source (`scripts/codex/config.autodev.toml`):** Contains the versioned, portable slice of configuration owned by AutoDev (model defaults, provider definitions, telemetry, feature flags, native agents, declared hooks, skills, and shell environment policy). MCP declarations are owned by `.rulesync/mcp.jsonc` and generated by the pinned Rulesync version.
