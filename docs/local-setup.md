...
`.rulesync/` is the only tracked Rulesync input, and nothing Rulesync generates is tracked as a test fixture. The supported providers are `codexcli`, `claudecode`, `copilot`, and `antigravity-cli`. `rulesync.jsonc` is the only Rulesync configuration: it generates live repository skill and hook projections described below.
