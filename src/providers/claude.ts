...
// stalled CLI.
const CLAUDE_TIMEOUT_SECONDS = Number.parseFloat(
  process.env.CLAUDE_CODE_BRIDGE_TIMEOUT_SECONDS ?? "900"
);
...
const CLAUDE_PARK_SECONDS = Number.parseFloat(
  process.env.CLAUDE_CODE_BRIDGE_PARK_SECONDS ?? "1800"
);
const CLI =
...
const _CLAUDE_EFFORT_LEVELS = new Set([
  "low",
  "medium",
  "high",
  "xhigh",
...
export class ClaudeRateLimitError extends Error {
  ...
  toLimit(): ProviderLimit {
    const limit: ProviderLimit = {
      limitClass: this.limitClass as ProviderLimit["limitClass"],
      limitType: this.limitType,
      source: this.source as LimitSource
    };
...
export function resolvedModel(requested: unknown): string {
  ...
  if (
    lowered.startsWith("claude-subscription") ||
...
export function resolveClaudeEffort(requested: unknown): string {
  ...
  return _CLAUDE_EFFORT_LEVELS.has(candidate)
    ? candidate
...
function requestedEffort(request: JsonRecord): unknown {
  ...
  if (reasoning && typeof reasoning === "object" && "effort" in reasoning)
    return (reasoning as JsonRecord).effort;
...
export function claudeModelMetadata(): JsonRecord {
  return {
    ...
    supported_reasoning_levels: ["low", "medium", "high"].map((effort) => ({
      effort,
      description: `Claude Code ${effort} reasoning`
    })),
...
export function rateLimitEventError(
  event: JsonRecord
): ClaudeRateLimitError | null {
...
export function raiseClassifiedClaudeError(
  message: unknown,
  errorCode: unknown
): void {
...
export function claudeEnvironment(): NodeJS.ProcessEnv {
  ...
  if (!environment.CLAUDE_CODE_OAUTH_TOKEN) {
    throw new Error(
      "CLAUDE_CODE_OAUTH_TOKEN is not available to the Claude bridge"
    );
  }
  return environment;
}
  ...
  feed(inner: JsonRecord): JsonRecord | null {
    ...
    if (inner.type === "content_block_start") {
      ...
      if (
        block &&
        typeof block === "object" &&
        (block as JsonRecord).type === "tool_use"
      ) {
    ...
    if (inner.type === "content_block_delta") {
      ...
      if (
        delta &&
        typeof delta === "object" &&
        (delta as JsonRecord).type === "input_json_delta" &&
        typeof index === "number"
      ) {
        this.open
          .get(index)?.[1]
          .push(String((delta as JsonRecord).partial_json ?? ""));
      }
      return null;
    }
    if (inner.type !== "content_block_stop" || typeof index !== "number")
      return null;
  ...
  const tools =
    surface.tools.length === 0
      ...
      : [
          ...
          ...(names.has("exec")
            ? [
                "`exec` runs JavaScript whose `tools` global carries the agent's full tool set -- shell commands, file patches, MCP servers, and, for an orchestrator, subagent management. Its description lists them. Read files and skills through these tools, as the instructions describe."
              ]
            : [])
        ].join(" ");
...
export function claudeCliArgs(
  ...
  const allowed = [
    ...builtIns,
    ...(options.turnId ? [`mcp__${CODEX_TOOLS_SERVER}`] : [])
  ];
  return [
    CLI,
    "-p",
...
export function describeFailure(error: unknown): TurnFailure {
  ...
  writeErrorLine(`Claude upstream failure: ${message}`);
  const limit =
    error instanceof ClaudeRateLimitError
      ? error.toLimit()
      : message
        ? classifyCliLimit(message)
        : null;
  const reason = limit
    ? INCOMPLETE_REASON_PROVIDER_LIMIT
...
async function handle(
  ...
  if (agentEvents) {
    for (const server of roleContract(agentRole).mcp ?? []) {
      void agentEvents.reportMcpExposed({
        server,
        source: CLAUDE_MCP_EXPOSURE_SOURCE
      });
    }
  }
  ...
  const stream = new ResponseStream({
    ...
    sendError: handleNonStreamingError
  });
  const reporter = turnReporter(agentEvents);
  const awaited = awaitedToolResults(payload.input);
  const parked =
    awaited.outputs.size > 0 ? turns.forOutputs(awaited.outputs) : null;
  if (parked) {
    writeErrorLine(
      `claude continuing turn=${parked.id} results=${awaited.outputs.size} messages=${awaited.messages.length}`
    );
  ...
  const turn = new ClaudeTurn({
    tools: surface.tools,
    registry: turns,
    parkMs: CLAUDE_PARK_SECONDS * 1000,
    describeFailure
  });
  writeErrorLine(
    `claude request model=${model} effort=${effort} role=${agentRole ?? "default"} cwd=${cwd} turn=${turn.id} tools=${JSON.stringify(surface.tools.map((tool) => tool.name))} web_search=${surface.webSearch}`
  );
  turn.attach(stream, reporter);
  void turn.run(
    runClaudeStream(renderCodexTranscript(payload.input ?? ""), model, effort, {
      cwd,
...
function handleNonStreamingError(
  ...
  if (error instanceof ClaudeRateLimitError) {
    ...
    sendJson(
      ...
      429,
      {
        error: { message, type: "rate_limit_error", limit: limitPayload(limit) }
      },
      extra
    );
    return;
...
if (IS_MAIN) {
  const server = createServer((request, response) => {
    ...
    writeErrorLine(
      `Claude Responses proxy listening at http://${HOST}:${PORT}`
    );
  });
  ...
  for (const signal of ["SIGTERM", "SIGINT"] as const) {
    process.once(signal, () => {
      ...
      if (cancelled > 0)
        writeErrorLine(
          `claude bridge stopping: cancelled ${cancelled} live turn(s)`
        );
      server.close();
      process.exit(0);
    });
