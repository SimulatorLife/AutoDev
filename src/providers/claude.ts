...
export function bridgeMcpServers(): Record<string, JsonRecord> {
  ...
  try {
    ...
    throw new ClaudeOverloadedError(`bridge MCP catalogue is invalid: ${path}; rerun install-codex-integration.sh`);
  } catch (error) {
    if (error instanceof ClaudeOverloadedError) throw error;
    throw new ClaudeOverloadedError(`bridge MCP catalogue is missing or invalid: ${path}; rerun install-codex-integration.sh (${(error as Error).message})`);
  }
}
...
export function claudeSkillViewForRole(role: unknown): string | null {
  ...
  if (!existsSync(join(view, ".claude", "skills"))) {
    throw new ClaudeOverloadedError(
      `Claude skill bootstrap view is missing for role ${roleKey}: ${view}; rerun install-codex-integration.sh`,
    );
  }
  return view;
}
...
export function mcpConfigForRole(role: unknown, spawnSession: string | null = null): string | null {
  ...
  for (const name of (contract.mcp ?? []) as string[]) {
    ...
    if (!server || typeof server !== "object") {
      throw new ClaudeOverloadedError(
        `MCP server ${JSON.stringify(name)} granted to role ${String(role)} is not in the bridge MCP catalogue; rerun install-codex-integration.sh`,
      );
    }
