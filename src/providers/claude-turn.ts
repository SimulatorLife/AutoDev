...
export class ResponseStream {
  ...
  private emit(eventName: string, body: JsonRecord): void {
    ...
    try {
      this.options.response.write(line);
    } catch {
      /* the close listener handles a gone client */
    }
  }
  ...
  keepAlive(): void {
    ...
    try {
      this.options.response.write(": claude-bridge keep-alive\n\n");
    } catch {
      /* ignore */
    }
  }
  ...
  private outputText(): string {
    return this.output
      ...
      .map(
        (item) =>
          ((item.content as JsonRecord[] | undefined)?.[0]?.text as
            string | undefined) ?? ""
      )
      .join("");
  }
  ...
  toolCall(item: JsonRecord): void {
    ...
    for (const [eventName, body] of codexToolCallEvents(item, index))
  ...
  fail(failure: TurnFailure, provider: string): void {
    ...
    this.end({
      ...this.snapshot("incomplete"),
      incomplete_details: incompleteDetails(failure.reason, failure.limit),
      output_text: this.outputText()
    });
  }
}
...
export class ClaudeTurn {
  ...
  attach(
    ...
    for (const [callId, output] of awaited.outputs) {
      ...
      reporter.toolExecuted(
        call.tool.name,
        callId,
        Date.now() - call.emittedAt,
        codexOutputFailed(output) ? "error" : "ok"
      );
    ...
    if (last && awaited.messages.length > 0) {
      (last.content as JsonRecord[]).push({
        type: "text",
        text: `Codex added to the conversation while this ran:\n\n${renderCodexTranscript(awaited.messages)}`
      });
    }
    ...
    for (const [callId, call] of this.awaiting) {
      ...
      call.resolve(
        toolErrorResult(
          `Codex returned no result for ${call.tool.name}; the call may not have run.`
        )
      );
    }
    this.scheduleGather();
  ...
  requestTool(name: string, args: unknown): Promise<JsonRecord> {
    ...
    if (!tool)
      return Promise.resolve(
        toolErrorResult(
          `Codex did not offer a tool named ${name} on this turn.`
        )
      );
    if (this.finished)
      return Promise.resolve(
  ...
  async run(events: AsyncIterable<ClaudeCliEvent>): Promise<void> {
    try {
      ...
      if (!this.finished)
        throw new Error("Claude CLI ended the turn without a result");
    } catch (error) {
  ...
  cancel(): void {
    ...
    if (this.stream)
      this.endStream({
        error: new Error("turn cancelled"),
  ...
  private complete(event: Extract<ClaudeCliEvent, { kind: "complete" }>): void {
    ...
    if (
      event.text &&
      event.text !== this.lastText &&
      !this.lastText.endsWith(event.text)
    ) {
      this.stream?.text(
        event.text.startsWith(this.lastText)
          ? event.text.slice(this.lastText.length)
          : event.text
      );
    }
    this.finished = true;
  ...
  private emitQueuedCalls(): void {
    ...
    for (const call of this.queued.splice(0)) {
      ...
      const item = codexToolCallItem(call.tool, call.args, {
        itemId: `${call.tool.kind === "custom" ? "ctc" : "fc"}_${hex(12)}`,
        callId
      });
      this.awaiting.set(callId, {
        tool: call.tool,
        emittedAt: Date.now(),
  ...
  private release(message: string): void {
    ...
    this.clearParkTimer();
    for (const call of this.queued.splice(0))
      call.resolve(toolErrorResult(message));
...
export class ClaudeTurnRegistry {
  ...
  remove(turn: ClaudeTurn): void {
    ...
    for (const [callId, owner] of this.calls)
      if (owner === turn) this.calls.delete(callId);
  }
