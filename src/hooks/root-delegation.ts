...
type HookInput = { model?: unknown; session_id?: unknown; hook_event_name?: unknown; turn_id?: unknown };
type SpawnTools = { buildRecoveryScript(parentId: string): string };

function parseInput(raw: string): HookInput {
  try {
    const value: unknown = JSON.parse(raw);
    return value !== null && typeof value === 'object' ? value as HookInput : {};
  } catch {
    return {};
  }
}
...
export async function runRootDelegation(raw = readFileSync(0, 'utf8')): Promise<number> {
  ...
  process.stdout.write(JSON.stringify({
    ...
    hookSpecificOutput: { hookEventName: 'UserPromptSubmit', additionalContext: prompt + skill + codeSearch + recoveryContext },
  }));
  return 0;
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  runRootDelegation().then((status) => { process.exitCode = status; });
}
