...
import { isDirectory, resolveCwd, WORKSPACE_KEYS } from '../shared/resolve-workspace.ts';
import { CodexStateCollector, loadCodexStateCollectorConfig } from './state-collector.ts';
import { COOLDOWNS, COOLDOWN_CONFIG, PROBE_FAILURE_CLASS } from './cooldown.ts';
import {
  ORCHESTRATOR_ALIAS,
...
import { createAgentActivityTracker, resolveAgentActivityTtlMs } from '../agents/agent-activity.ts';
import {
  ConcurrencyManager,
...
import { dropUnresolvableReasoning, normalizeInputItemIds } from '../shared/responses-item-ids.ts';

export { errorBody, sendJson } from './proxy.ts';

export const HOST = process.env.CODEX_MODEL_ROUTER_HOST ?? '127.0.0.1';
export const PORT = Number.parseInt(process.env.CODEX_MODEL_ROUTER_PORT ?? '4100', 10);
export const AGENT_ACTIVITY_TTL_MS = resolveAgentActivityTtlMs();
export const agentActivity = createAgentActivityTracker({ ttlMs: AGENT_ACTIVITY_TTL_MS });
...
const dashboardSource = new URL('../../scripts/codex-model-router-dashboard.html', import.meta.url);
const dashboardInstalled = new URL('../../hooks/codex-model-router-dashboard.html', import.meta.url);
const DASHBOARD_FILE = existsSync(dashboardSource) ? dashboardSource : dashboardInstalled;
const ROUTER_STARTED_AT = new Date().toISOString();
const SHUTDOWN_DRAIN_TIMEOUT_MS = Number.parseInt(process.env.CODEX_ROUTER_SHUTDOWN_DRAIN_MS ?? '30000', 10);

export const providerTelemetry = new Map(ROUTES.map(({ provider }) => [provider, {
  attempts: 0,
  successes: 0,
  failures: 0,
  skipped: 0,
  ...
  lastFailure: null as { timestamp: string; class: string | null; status: number | null | undefined } | null,
}]));
  ...
  onEvent: (event, input, effectiveOrigin) => {
    ...
    if (event.provider && event.model && ['selected', 'skipped', 'result'].includes(event.phase)) {
      recordUsageEvent({
        ...
        origin: effectiveOrigin ?? usageOrigin(event.role, event.provider as any),
      } as any);
    }
    if (event.phase === 'result') closeBridgeSubagentsForRequest(event.requestId as any, event.outcome as any, event.elapsedMs);
...
const subagentRegistry = new SubagentRegistry({
  ...
  getSpawnCapableProviders: () => Object.keys(ROUTING.providers).filter((provider) => providerCapabilities(provider).subagentSpawn),
});
...
const otelTracker = new OtelTracker({
  ...
  getConversationThread: (id) => (codexState.lastSnapshot && (codexState.lastSnapshot as any).conversationThreads?.[id]) ?? null,
...
setUpstreamShapeHooks({
  ...
  recordEvent: (event) => {
    ...
    recordRouterEvent({
      ...
      droppedReasoningItems: typeof event.droppedReasoningItems === 'number' ? event.droppedReasoningItems : 0,
      normalizedItemIds: typeof event.normalizedItemIds === 'number' ? event.normalizedItemIds : 0,
    });
  },
});
...
export async function refreshCodexState(): Promise<void> {
  try {
    codexState.lastSnapshot = (await codexState.collector.collectSnapshot()) as unknown as Record<string, unknown>;
  } catch (error) {
...
export function setCodexStateSnapshotForTests(snapshot: Record<string, unknown> | null): void {
...
export function codexStateStatus(): Record<string, unknown> {
  ...
  const { path: _path, ...safeLocalTelemetry } = (snapshot.localTelemetry as Record<string, unknown>) ?? {};
  return {
    localTelemetry: { ...safeLocalTelemetry, pathConfigured: Boolean((snapshot.localTelemetry as Record<string, unknown>)?.path) },
...
const routerPersistence = new RouterPersistence({
  ...
  isMain: Boolean(process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]),
  debounceMs: 500,
  getSnapshot: () => ({
    ...
    if (section === 'providerTelemetry') {
      ...
      if (!parsed.usage) {
        ...
        for (const event of routerEvents.getRecentEvents()) {
          if (event.provider && event.model && event.phase) recordUsageEvent(event as any);
        }
        inFlightUsage.clear();
      }
    }
      ...
      for (const [tier, groups] of Object.entries(ROUTING.providerGroups)) {
        if (Array.isArray(groups)) {
          for (let gIdx = 0; gIdx < groups.length; gIdx++) {
            ...
            if (Array.isArray(group) && group.some((name) => String(name).toLowerCase() === route.provider.toLowerCase())) {
...
export function limitsStatus(): Record<string, unknown> {
  return {
    ...
    maxConcurrentThreadsPerSession: getDefaultConcurrencyManager().effectivePerSessionLimit(),
  };
}
...
export function agentsStatus(at = Date.now()): Record<string, unknown> {
  ...
  for (const agent of liveAgents) {
    const kind = typeof agent.kind === 'string' && agent.kind ? agent.kind : 'session';
  ...
  return {
    ...
    slotVsAgent: {
      ...
      admissionSlots: getDefaultUsageTracker().activityTracker.countLive({ kind: SUBAGENT_SLOT_KIND }, at),
      activeAdmissionSessions: getDefaultUsageTracker().activityTracker.distinctTags({ kind: SUBAGENT_SLOT_KIND }, at).length,
      processFallbackActiveThreads: getDefaultUsageTracker().activityTracker.countLive({ kind: SUBAGENT_SLOT_KIND, tag: PROCESS_FALLBACK_SESSION_KEY }, at),
    },
...
export function recordConcurrencyDenial(denial: unknown): void {
  getDefaultConcurrencyManager().recordConcurrencyDenial(denial as Parameters<ConcurrencyManager['recordConcurrencyDenial']>[0]);
  const info = typeof denial === 'string' ? { reason: denial } : ((denial as Record<string, unknown>) ?? {});
  recordSpawnFailure({
...
export const INGESTED_AGENT_EVENTS = Object.freeze(new Set([
  'subagent_spawn',
  'subagent_result',
...
export function ingestAgentEvents(payload: Record<string, unknown>): {
  ...
  let context = requestId ? getDefaultSubagentRegistry().getBridgeRequestContext(requestId) : undefined;
  ...
  for (const event of events) {
    if (!event || typeof event !== 'object' || !INGESTED_AGENT_EVENTS.has(event.type as string)) {
      rejected += 1;
      continue;
    }
    ...
    if (event.type === 'subagent_result') {
      ...
      const durationMs = Number.isFinite(event.durationMs) ? Math.max(0, event.durationMs as number) : null;
    ...
    for (const child of children) {
      openBridgeSubagentUsage({ requestId, context, role, childId: child.id, model: child.model });
    }
    accepted += count;
  }
...
export function getRouterStatus(now = Date.now()): Record<string, unknown> {
  ...
  return {
    ...
    telemetryPersistence: {
      enabled: Boolean(process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]),
...
export function parseTurnMetadataJson(value: unknown): Record<string, unknown> | null {
...
export function hasWorkspaceClaim(payload: Record<string, unknown> | null | undefined, turnMetadataHeader: string | null): boolean {
  ...
  const explicitPaths = [
    ...
    ...(metadata && typeof metadata === 'object' ? WORKSPACE_KEYS.map((key) => metadata[key]) : []),
  ];
...
export function workspaceContextFromRequest(
  ...
  const path = (explicitPaths.find((value) => typeof value === 'string' && value.trim()) as string | undefined)
  ...
  const matchingEntry = path && workspaces[path]
    ...
    : resolvableKeys.length === 1
      ...
      : workspaceKeys.length === 1
        ...
        : null;
  const remotes = matchingEntry?.associated_remote_urls as Record<string, string> | undefined;
  ...
  const rawId = (matchingEntry?.workspace_id as string | undefined)
  ...
  const context: { key: string; cwd: string | null; workspace_id?: string } = { key, cwd };
...
export function sendRouterAuthFailure(response: ServerResponse): void {
  sendJson(
    response,
    401,
    errorBody('Router authentication is required.', 'router_authentication_error', {
...
export async function loadCatalog(catalogFile = CATALOG_FILE): Promise<{ models: unknown[]; data: unknown[] }> {
  try {
    const parsed = JSON.parse(await readFile(catalogFile, 'utf8'));
    const models = Array.isArray(parsed.models) ? parsed.models : [];
    return {
      models,
      data: models.map((model: { slug: string }) => ROUTING_POLICY.providerModelMetadata(model.slug)),
    };
  } catch {
    return { models: [], data: [] };
  }
...
export async function handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (pathname === '/health/readiness') {
    ...
    sendJson(response, 200, { status: 'ready', router: 'codex-model-router', lifecycle: getDefaultRouterLifecycle().getLifecycleStatus() });
    return;
  }
  ...
  if (getDefaultRouterLifecycle().isDraining()) {
    sendJson(
      response,
      503,
      errorBody('Router is draining for shutdown; please retry against another instance.', 'router_draining', { code: 'router_draining', retryable: true }),
      { 'retry-after': '5' },
    );
    return;
  }
  ...
  try {
    ...
    if (role) {
      const denialReason = getDefaultConcurrencyManager().tryAcquireSubagentSlot(session.key);
      if (denialReason) {
        recordConcurrencyDenial({ requestId, role, requestedModel: payload.model, sessionScope: session.scope, reason: denialReason });
        sendJson(
          response,
          429,
          errorBody(`Subagent denied by configured ${denialReason} limit.`, 'router_concurrency_limit', {
  ...
  try {
    ...
    try {
      if (response.headersSent) {
        ...
        sendJson(response, 502, errorBody('The router could not complete the request.', 'router_internal_error', { code: 'router_internal_error', retryable: true }));
      }
    } catch {
