...
export const ROUTER_AUTH_REQUIRED_MESSAGE = 'Router authentication is required.';
...
export function resolveRouterAuthToken(env: NodeJS.ProcessEnv = process.env): string {
...
export function isRouterAuthEnabled(configuredToken: string = getRouterAuthToken()): boolean {
  return Boolean(configuredToken);
}

export function authStatus(configuredToken: string = getRouterAuthToken()): { responseRequests: boolean } {
