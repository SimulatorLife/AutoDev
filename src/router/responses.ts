...
export type FlattenedNamespace = { readonly namespace: string; readonly prefix: string };
...
export type RouterResponseItem = RouterResponseMessageItem | {
  type: string;
  id?: string;
  role?: string;
  status?: string;
...
export interface UpstreamPayloadOptions {
  ...
  dropUnresolvableReasoning?: (input: unknown) => { input: unknown; dropped: number };
  normalizeInputItemIds?: (input: unknown) => { input: unknown; changed: number };
...
export function flattenOutboundTool(tool: unknown, defaultNamespace: string | null = null): unknown {
  if (!isRecord(tool)) return tool;
  const ns = (typeof tool.namespace === 'string' ? tool.namespace : defaultNamespace) ?? null;
  ...
  if (prefix) {
    ...
    if (isRecord(result.function) && typeof result.function.name === 'string' && !result.function.name.startsWith(prefix)) {
      result.function = { ...result.function, name: `${prefix}${result.function.name}` };
    }
  }
  return result;
}
  ...
  for (const item of tools) {
    ...
    if (typeof ns === 'string' && Array.isArray(item.tools)) {
      for (const innerTool of item.tools) {
        if (isRecord(innerTool)) flattened.push(flattenOutboundTool(innerTool, ns));
      }
    } else {
...
export function rewriteToolNamespaces(value: unknown): unknown {
  if (Array.isArray(value)) return value.map((item) => rewriteToolNamespaces(item));
  if (value === null || !isRecord(value)) return value;
  const result: Record<string, unknown> = {};
  for (const [key, child] of Object.entries(value)) result[key] = rewriteToolNamespaces(child)!;
  if (typeof result.name === 'string' && (result.namespace === undefined || result.namespace === null)) {
    const match = FLATTENED_NAMESPACES.find(([, prefix]) => typeof result.name === 'string' && result.name.startsWith(prefix));
...
export function replaceModelFields(value: unknown, publicModel: string): unknown {
  if (Array.isArray(value)) return value.map((item) => replaceModelFields(item, publicModel));
  if (value === null || !isRecord(value)) return value;
  const result: Record<string, unknown> = {};
  for (const [key, item] of Object.entries(value)) {
    result[key] = key === 'model' && typeof item === 'string' ? publicModel : replaceModelFields(item, publicModel);
  }
  return result;
}

export function rewriteResponseValue(value: unknown, publicModel: string): unknown {
...
export function countToolCallsInResponse(response: unknown, seen: Set<string> = new Set()): number {
  ...
  for (const item of response.output) {
    if (isRecord(item) && typeof item.type === 'string' && TOOL_OUTPUT_TYPES.has(item.type) && typeof item.id === 'string' && !seen.has(item.id)) {
      seen.add(item.id);
      count += 1;
    }
  }
...
export function countToolCallsFromSse(body: string, seen: Set<string> = new Set()): number {
  let count = 0;
  for (const line of body.split(/\r?\n/)) {
    if (!line.startsWith('data: ') || line.slice(6) === '[DONE]') continue;
    try {
      const event = JSON.parse(line.slice(6));
      if (event.type === 'response.output_item.added' && isRecord(event.item) && typeof event.item.type === 'string' && TOOL_OUTPUT_TYPES.has(event.item.type) && typeof event.item.id === 'string' && !seen.has(event.item.id)) {
...
export function responseTextFromSse(body: string): RouterResponseEnvelope {
  ...
  return buildCompletedResponse(text, isRecord(completed) ? { completedResponse: completed } : {});
}

export function buildCompletedResponse(text: string, extras: { id?: string; completedResponse?: unknown } = {}): RouterResponseEnvelope {
  ...
  if (isRecord(completed)) {
    return {
      ...
      output: Array.isArray(completed.output) && completed.output.length
        ? completed.output as RouterResponseItem[]
        : completedEnvelopeMessage(text),
    } as RouterResponseEnvelope;
  }
  return {
    ...(extras.id !== undefined ? { id: extras.id } : { id: `router_${Date.now()}` }),
    object: 'response',
    status: 'completed',
    output_text: text,
...
export function setUpstreamShapeHooks(hooks: Partial<UpstreamShapeHooks>): void {
 ...
 */
function normalizePayloadInput(route: RouterProviderRouteLike, payload: Record<string, unknown>, requestId: string | null | undefined, hooks: UpstreamShapeHooks): Record<string, unknown> {
  const result: Record<string, unknown> = { ...payload };
  if (route.provider === 'codex') {
    if (hooks.shouldDropUnresolvableReasoning) {
...
export function upstreamPayload(
  ...
  const { extra_headers: _discarded, ...safePayload } = payload as { extra_headers?: unknown } & Record<string, unknown>;
  void _discarded;
  const normalizeItemIds = options.normalizeItemIds ?? hooks.shouldNormalizeItemIds ?? true;
  const hookOptions: UpstreamShapeHooks = {
    ...hooks,
    shouldNormalizeItemIds: normalizeItemIds,
  };
  const prepared = normalizeItemIds
    ? normalizePayloadInput(route, safePayload as Record<string, unknown>, requestId, hookOptions)
