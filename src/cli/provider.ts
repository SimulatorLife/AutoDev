...
const unmigratedProvider: ProviderCommandBackend = {
  start: (provider) => { throw new UnmigratedRuntimeError(`provider ${provider}`); },
};

export function dispatchProviderCommand(name: string, backend: ProviderCommandBackend = unmigratedProvider): number {
  if (name !== 'claude' && name !== 'minimax' && name !== 'copilot' && name !== 'antigravity') {
