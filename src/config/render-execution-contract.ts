...
import { atomicWriteJson, ConfigError, parseArgs, readJsonFile, requiredArg, type TomlTable } from './toml.ts';
...
function sortedByOrder(values: string[], order: Record<string, number>): string[] {
...
function readRole(source: string): RoleProjection {
  ...
  const enabledServers = Object.entries(servers).filter(([, value]) => record(value).enabled === true);
  const mcp = sortedByOrder(enabledServers.map(([name]) => name), MCP_ORDER);
  ...
  const skills = Array.isArray(skillsTable.config)
    ? skillsTable.config
      ...
      .filter((entry) => entry.enabled === true && typeof entry.name === 'string')
  ...
  if (tools !== undefined) {
    ...
    if (tools === null || typeof tools !== 'object' || Array.isArray(tools)) throw new ConfigError(`${source}: tools must be a table`);
    if (toolConfig.web_search !== undefined && typeof toolConfig.web_search !== 'boolean') throw new ConfigError(`${source}: tools.web_search must be boolean`);
    if (toolConfig.web_search === true) webResearch = { search: true, fetch: true, optionalMcp: [] };
  }

  const roleName = source.split('/').pop()?.replace(/\.toml$/u, '') ?? '';
  const kind = text.split('\n').some((line) => line.trim() === '# role-kind: orchestrator') ? 'orchestrator' : 'leaf';
...
export function renderExecutionContract(sourceDir: string, rootConfigPath: string, contractPath: string): ExecutionContract {
  const template = readJsonFile(contractPath, 'execution contract');
  const roles: Record<string, RoleProjection> = {};
  for (const entry of readdirSync(sourceDir, { withFileTypes: true }).filter((item) => item.isFile() && item.name.endsWith('.toml')).sort((a, b) => a.name.localeCompare(b.name))) {
    roles[entry.name.slice(0, -'.toml'.length)] = readRole(join(sourceDir, entry.name));
  }

  for (const roleName of RESEARCH_ROLES) {
    const webResearch = roles[roleName]?.webResearch;
    if (!webResearch?.search || !webResearch.fetch) throw new ConfigError(`role '${roleName}' must declare webResearch with search and fetch enabled`);
  }
  if (!roles.orchestrator) throw new ConfigError('role TOMLs must include the orchestrator capability declaration');

  const rootConfig = record(parseToml(readFileSync(rootConfigPath, 'utf8')));
  const rootServers = record(rootConfig.mcp_servers);
  const enabledRootMcp = new Set(Object.entries(rootServers).filter(([, value]) => record(value).enabled !== false).map(([name]) => name));
  const missingRootMcp = roles.orchestrator.mcp.filter((name) => name !== 'autodev_spawn' && !enabledRootMcp.has(name));
  if (missingRootMcp.length > 0) throw new ConfigError(`orchestrator capability TOML is not enabled in root config: ${missingRootMcp.sort().join(', ')}`);
...
export function runExecutionContract(sourceDir: string, rootConfigPath: string, contractPath: string, output: string): number {
  atomicWriteJson(output, renderExecutionContract(sourceDir, rootConfigPath, contractPath));
  console.log(`rendered execution contract into ${output}`);
  return 0;
}
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  try {
    ...
    console.error(`render-execution-contract: ${error instanceof Error ? error.message : error}`);
    process.exitCode = 2;
  }
}
