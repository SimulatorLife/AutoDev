...
export interface ServiceRestartDeps {
  readonly launchd: Pick<LaunchdClient, 'isLoaded' | 'print' | 'bootout' | 'bootstrap' | 'enable' | 'kickstart'>;
  ...
  readonly run: (command: string, args: readonly string[], input?: string, env?: NodeJS.ProcessEnv) => number;
...
const SERVICE_PORTS: Record<ManagedServiceLabel, number> = {
  ...
  'com.codex.minimax-proxy': 18765,
  'com.codex.antigravity-proxy': 4002,
  'com.codex.copilot-proxy': 4003,
  'com.codex.otel-collector': 4318,
};

export function resolveServiceRestartOptions(env: NodeJS.ProcessEnv = process.env): ServiceRestartOptions {
  ...
  return {
    ...
    readyAttempts: positiveInteger(env.AUTODEV_SERVICE_READY_ATTEMPTS, DEFAULT_ATTEMPTS),
    readyDelayMs: positiveInteger(env.AUTODEV_SERVICE_READY_DELAY_MS, DEFAULT_DELAY_MS),
  };
}
...
function defaultDeps(): ServiceRestartDeps {
  return {
    ...
    readFile: (path) => { try { return readFileSync(path, 'utf8'); } catch { return ''; } },
    commandAvailable: (command) => { try { execFileSync('which', [command], { stdio: 'ignore' }); return true; } catch { return false; } },
    probe: async (url, method = 'GET') => {
      try { return (await fetch(url, { method, signal: AbortSignal.timeout(1_000) })).ok; } catch { return false; }
    },
    sleep: (ms) => new Promise((resolve) => setTimeout(resolve, ms)),
    run: (command, args, input, env) => {
      const stdio: 'inherit' | ['pipe', 'inherit', 'inherit'] = input === undefined ? 'inherit' : ['pipe', 'inherit', 'inherit'];
      return spawnSync(command, [...args], { input, env: { ...process.env, ...env }, stdio }).status ?? 1;
    },
    listeningPids: (port) => {
      try {
        const output = execFileSync('lsof', ['-nP', '-tiTCP:' + port, '-sTCP:LISTEN'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
        return output.split(/\s+/).filter((pid) => /^[0-9]+$/.test(pid)).map(Number);
      } catch { return null; }
    },
    commandLine: (pid) => { try { return execFileSync('ps', ['-o', 'command=', '-p', String(pid)], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim(); } catch { return null; } },
    kill: (pid) => { try { process.kill(pid); } catch { /* process exited */ } },
  };
}

function plistPath(options: ServiceRestartOptions, label: ManagedServiceLabel): string {
  return join(options.home, 'Library', 'LaunchAgents', `${label}.plist`);
}

function serviceLauncher(options: ServiceRestartOptions, label: ManagedServiceLabel): string {
  const hooks = join(options.codexHome, 'hooks');
  switch (label) {
    case 'com.codex.model-router': return join(hooks, 'run-codex-model-router.sh');
    case 'com.codex.claude-bridge': return join(hooks, 'run-codex-claude-bridge.sh');
...
function serviceHook(options: ServiceRestartOptions, label: ManagedServiceLabel): string {
  switch (label) {
    ...
    case 'com.codex.minimax-proxy': return join(options.codexHome, 'src', 'providers', 'minimax.ts');
    case 'com.codex.otel-collector': return serviceLauncher(options, label);
  }
}

function serviceProbe(label: ManagedServiceLabel): string {
  if (label === 'com.codex.model-router') return 'http://127.0.0.1:4100/health/readiness';
  if (label === 'com.codex.claude-bridge') return 'http://127.0.0.1:4000/health/liveliness';
  if (label === 'com.codex.antigravity-proxy') return 'http://127.0.0.1:4002/health/liveliness';
  if (label === 'com.codex.copilot-proxy') return 'http://127.0.0.1:4003/health/liveliness';
  return 'http://127.0.0.1:18765/health';
}

function plistOwner(options: ServiceRestartOptions, deps: ServiceRestartDeps): string | null {
  const path = plistPath(options, 'com.codex.model-router');
  if (!deps.fileExists(path)) return null;
  const match = /<key>CODEX_HOME<\/key>\s*<string>([^<]+)<\/string>/u.exec(deps.readFile(path));
  return match?.[1] ?? null;
}

async function waitForProbe(deps: ServiceRestartDeps, url: string, options: ServiceRestartOptions, method: 'GET' | 'POST' = 'GET'): Promise<boolean> {
...
function reapUnmanaged(options: ServiceRestartOptions, label: ManagedServiceLabel, deps: ServiceRestartDeps): void {
  ...
  for (const pid of pids) {
    if (deps.commandLine(pid)?.includes(hook)) {
      console.error(`reaping unmanaged ${label} on port ${SERVICE_PORTS[label]} (pid ${pid})`);
      deps.kill(pid);
    } else {
      console.error(`port ${SERVICE_PORTS[label]} held by a process this installer does not own (pid ${pid}); ${label} not started`);
    }
  }
}

async function runDirectEnsures(options: ServiceRestartOptions, deps: ServiceRestartDeps): Promise<number> {
  ...
  for (const [model, script] of [
    ...
    if (run(join(options.repositoryRoot, 'scripts', script), `{"model":"${model}"}\n`) !== 0) console.error(`bridge start failed: ${script} (router will route around it)`);
  }
  if (run(join(options.repositoryRoot, 'scripts', 'ensure-codex-copilot-proxy.sh')) !== 0) console.error('bridge start failed: ensure-codex-copilot-proxy.sh (router will route around it)');
  return 0;
}
...
export async function restartServices(options: ServiceRestartOptions = resolveServiceRestartOptions(), deps: ServiceRestartDeps = defaultDeps()): Promise<number> {
  ...
  if (owner && join(options.codexHome, 'hooks') !== join(owner, 'hooks')) {
    console.error(`materialized into ${join(options.codexHome, 'hooks')}; leaving the services under ${join(owner, 'hooks')} alone.`);
    return 0;
  }
  ...
  for (const label of MANAGED_SERVICE_LABELS) {
    ...
    if (label === 'com.codex.otel-collector' && options.otelMode === 'direct') {
      try { deps.launchd.bootout(label); } catch { /* not loaded */ }
      continue;
    }
    const plist = plistPath(options, label);
    if (!deps.fileExists(plist)) { launchdAvailable = false; continue; }
    try { deps.launchd.bootout(label); } catch { /* not loaded */ }
    reapUnmanaged(options, label, deps);
    try {
      deps.launchd.bootstrap(plist);
      try { deps.launchd.enable(label); } catch { /* best effort */ }
      try { deps.launchd.kickstart(label); } catch { launchdAvailable = false; }
    } catch { launchdAvailable = false; }
  }
  if (foreignService) {
    console.error('another AutoDev runtime owns one or more labels; leaving all active services untouched.');
    return 0;
  }
  if (launchdAvailable) {
    console.error('Provider bridges supervised by launchd (KeepAlive; survive restart/crash/sleep).');
    for (const label of MANAGED_SERVICE_LABELS.slice(0, 5)) await waitForProbe(deps, serviceProbe(label), options);
    if (options.otelMode === 'collector') await waitForProbe(deps, 'http://127.0.0.1:4318/v1/logs', options, 'POST');
  } else {
    console.error('launchctl unavailable (sandbox?); starting bridges through the direct ensure-hook path.');
  }
  return runDirectEnsures(options, deps);
}

if (process.argv[1] === new URL(import.meta.url).pathname) {
  restartServices().then((status) => { process.exitCode = status; });
}
