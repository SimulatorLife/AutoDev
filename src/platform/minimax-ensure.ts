...
function resolveNode(env: NodeJS.ProcessEnv): string {
  ...
  try { return execFileSync('which', ['node'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim() || 'node'; } catch { return 'node'; }
}

export function isMiniMaxModel(input: string, expectedModel = 'MiniMax-M3'): boolean {
  try {
...
export function resolveMiniMaxEnsureOptions(env: NodeJS.ProcessEnv = process.env): MiniMaxEnsureOptions {
  const home = env.HOME?.trim() || homedir();
  const codexHome = env.CODEX_HOME?.trim() || join(home, '.codex');
  const port = positiveInteger(env.CODEX_MINIMAX_PROXY_PORT, 18765);
  return {
    ...
    plist: join(home, 'Library', 'LaunchAgents', 'com.codex.minimax-proxy.plist'),
    launcher: join(codexHome, 'hooks', 'ensure-codex-minimax-proxy.sh'),
    proxyScript: join(codexHome, 'src', 'providers', 'minimax.ts'),
    nodeBin: resolveNode(env),
    upstreamBaseUrl: env.CODEX_MINIMAX_UPSTREAM_URL?.trim() || 'https://api.minimax.io',
    readyTimeoutMs: positiveInteger(env.CODEX_MINIMAX_READY_TIMEOUT_MS, DEFAULT_TIMEOUT_MS),
    logPath: env.CODEX_MINIMAX_PROXY_LOG?.trim() || join(codexHome, 'run', `codex-minimax-proxy-${port}.log`),
  };
}
...
function defaultDeps(options: MiniMaxEnsureOptions): MiniMaxEnsureDeps {
  ...
  return {
    ...
    startFallback: (current) => {
      const runDir = dirname(current.logPath);
      ...
      const child = spawn(current.nodeBin, [current.proxyScript], {
        detached: true,
        env: { ...process.env, MINIMAX_PROXY_HOST: current.host, MINIMAX_PROXY_PORT: String(current.port), MINIMAX_PROXY_UPSTREAM_BASE_URL: current.upstreamBaseUrl },
        stdio: ['ignore', fd, fd],
      });
      child.unref();
    },
...
async function waitForProbe(deps: MiniMaxEnsureDeps, timeoutMs: number): Promise<boolean> {
...
export async function ensureMiniMaxProxy(
  ...
  if (!deps.nodeAvailable()) {
    console.error('MiniMax compatibility proxy requires Node.js.');
    return 127;
  }
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ensureMiniMaxProxy().then((status) => { process.exitCode = status; });
}
