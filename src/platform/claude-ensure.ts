  ...
  try {
    return execFileSync(
      ...
      [
        ...
        env.USER ?? "",
        "-s",
        "com.codex.claude-bridge.oauth-token",
        "-w"
      ],
      {
        encoding: "utf8",
...
export function resolveClaudeEnsureOptions(
  ...
  return {
    ...
    logPath: path.join(codexHome, "run", "codex-claude-bridge.fallback.log")
  };
}
...
export async function ensureClaudeBridge(
  ...
  if (!deps.tokenAvailable()) {
    writeErrorLine(
      "Claude Code bridge requires a Keychain-backed Claude OAuth token."
    );
    return 1;
  }
  ...
  if (deps.launchd.isLoaded(options.label)) {
    ...
    writeErrorLine(
      `Claude launchd service ${options.label} did not become ready.`
    );
    return 1;
  }
  ...
  if (!deps.launcherExists()) {
    writeErrorLine(`Claude bridge launcher is missing: ${options.launcher}`);
    return 1;
  }
