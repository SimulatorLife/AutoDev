...
import { chmod as chmodP, lstat as lstatP, mkdir as mkdirP, readFile as readFileP, rename as renameP, rm as rmP, stat as statP, writeFile as writeFileP } from 'node:fs/promises';
import { existsSync, openSync, rmSync, rmdirSync } from 'node:fs';
...
export interface StartedLauncher { readonly pid: number }
...
export interface RouterEnsureDeps {
  ...
  readonly startLauncher: (command: string, args: string[], logPath: string) => StartedLauncher;
  ...
  readonly lstat: (path: string) => Promise<{ isSymbolicLink(): boolean } | null>;
  readonly writeFile: (path: string, data: string) => Promise<void>;
  readonly readFile: (path: string) => Promise<string>;
  readonly unlink: (path: string) => Promise<void>;
  readonly rename: (from: string, to: string) => Promise<void>;
  readonly stat: (path: string) => Promise<{ size: number } | null>;
...
const MAX_BACKOFF_MS_DEFAULT = 1_000;
...
function resolveRouterEnsureOptionsFromEnv(env: NodeJS.ProcessEnv, myPid: number): RouterEnsureOptions {
  const codexHome = env.CODEX_HOME?.trim() || join(homedir(), '.codex');
  const runDir = env.CODEX_MODEL_ROUTER_RUN_DIR?.trim() || join(codexHome, 'run');
  const launchdRunDir = join(codexHome, 'run');
  const ensureLock = env.CODEX_MODEL_ROUTER_ENSURE_LOCK?.trim() || join(runDir, 'codex-model-router.ensure.lock');
  const fallbackLog = env.CODEX_MODEL_ROUTER_FALLBACK_LOG?.trim() || join(runDir, 'codex-model-router.fallback.log');
  const fallbackPidFile = env.CODEX_MODEL_ROUTER_FALLBACK_PID_FILE?.trim() || join(runDir, 'codex-model-router.fallback.pid');
  const label = DEFAULT_LAUNCHD_LABEL;
  const plistLink = join(homedir(), 'Library', 'LaunchAgents', `${label}.plist`);
  ...
  return {
    paths: {
      ...
      fallbackLog,
      fallbackPidFile,
      launcher: join(codexHome, 'hooks', 'run-codex-model-router.sh'),
      plistLink,
      launchdLogOut: join(launchdRunDir, 'codex-model-router.launchd.out.log'),
      launchdLogErr: join(launchdRunDir, 'codex-model-router.launchd.err.log'),
    },
    label,
    domain,
    routerHost: env.CODEX_MODEL_ROUTER_HOST?.trim() || '127.0.0.1',
    routerPort: positiveInteger(env.CODEX_MODEL_ROUTER_PORT, 4100),
    readyTimeoutMs: positiveInteger(env.CODEX_MODEL_ROUTER_READY_TIMEOUT_MS, READY_TIMEOUT_MS_DEFAULT),
...
function readPsCommandLine(pid: number): string | null {
  try {
    const stdout = execFileSync('ps', ['-p', String(pid), '-o', 'command='], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
    return stdout.trim();
  } catch {
    return null;
  }
...
function readListenerPid(port: number): number | null {
  try {
    const stdout = execFileSync('lsof', ['-nP', '-a', `-iTCP:${port}`, '-sTCP:LISTEN', '-t'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
...
function defaultStartLauncher(command: string, args: readonly string[], logPath: string): StartedLauncher {
...
export function createDefaultRouterEnsureDeps(options: RouterEnsureOptions): RouterEnsureDeps {
  const probeUrl = `http://${options.routerHost}:${options.routerPort}/health/liveliness`;
  return {
    launchd: new LaunchdClient(),
    probe: async () => {
      try {
        const res = await fetch(probeUrl, { signal: AbortSignal.timeout(1_000) });
        return res.ok;
      } catch {
        return false;
      }
    ...
    signalPid: (pid, signal) => { try { process.kill(pid, signal); return true; } catch { return false; } },
    sleep: (ms) => new Promise((resolve) => setTimeout(resolve, ms)),
    now: () => Date.now(),
    mkdir: async (path, opts) => { await mkdirP(path, opts); },
    chmod: (path, mode) => chmodP(path, mode),
    lstat: async (path) => {
      try {
    ...
    writeFile: (path, data) => writeFileP(path, data, { encoding: 'utf8' }),
    readFile: async (path) => {
      try {
        return await readFileP(path, { encoding: 'utf8' });
      } catch {
        return '';
      }
    },
    unlink: async (path) => { try { await rmP(path, { recursive: true, force: true }); } catch { /* ignore */ } },
    rename: async (from, to) => { try { await renameP(from, to); } catch { /* ignore */ } },
    stat: async (path) => {
      try {
        const s = await statP(path);
        return { size: s.size };
      } catch {
        return null;
      }
...
async function waitForProbe(deps: RouterEnsureDeps, options: RouterEnsureOptions): Promise<boolean> {
...
async function secureLogFile(deps: RouterEnsureDeps, path: string): Promise<void> {
  const stat = await deps.lstat(path);
  if (stat && stat.isSymbolicLink()) {
    throw new Error(`refusing to use symlinked log path: ${path}`);
  }
  if (!stat) await deps.writeFile(path, '');
  await deps.chmod(path, 0o600);
}

async function acquireLock(deps: RouterEnsureDeps, options: RouterEnsureOptions): Promise<boolean> {
  const { paths } = options;
  ...
  await deps.mkdir(dirname(paths.ensureLock), { recursive: true });
  ...
  const ownerRaw = await deps.readFile(join(paths.lockDir, 'pid'));
  const owner = /^[0-9]+$/.test(ownerRaw.trim()) ? Number.parseInt(ownerRaw.trim(), 10) : NaN;
  if (Number.isFinite(owner) && !deps.pidExists(owner)) {
    ...
    try {
      ...
      await deps.unlink(join(stale, 'pid'));
...
async function tryMakeLockDir(deps: RouterEnsureDeps, lockDir: string, pid: number): Promise<boolean> {
  try {
  ...
  await deps.writeFile(join(lockDir, 'pid'), `${pid}\n`);
  await deps.chmod(join(lockDir, 'pid'), 0o600);
  return true;
}

function launchdOwnsListener(deps: RouterEnsureDeps, options: RouterEnsureOptions, launchdPid: number | null): boolean {
...
function fallbackPidOwned(deps: RouterEnsureDeps, pid: number): boolean {
  ...
  return cmd.includes('run-codex-model-router.sh') || cmd.includes('codex-model-router.mjs');
}

async function rotateFallbackLog(deps: RouterEnsureDeps, options: RouterEnsureOptions): Promise<void> {
  const stat = await deps.stat(options.paths.fallbackLog);
  if (stat && stat.size > options.fallbackLogMaxBytes) {
    try { await deps.rename(options.paths.fallbackLog, `${options.paths.fallbackLog}.1`); } catch { /* ignore */ }
  }
}
...
function safeLaunchctlPrint(deps: RouterEnsureDeps, options: RouterEnsureOptions): string | null {
  try { return deps.launchd.print(options.label); } catch { return null; }
}

async function ensureViaLaunchd(deps: RouterEnsureDeps, options: RouterEnsureOptions): Promise<0 | 1 | 2> {
  ...
  if (deps.launchd.isLoaded(options.label)) {
    ...
    try { deps.launchd.kickstart(options.label); } catch { return 1; }
...
async function ensureViaFallback(deps: RouterEnsureDeps, options: RouterEnsureOptions): Promise<0 | 2 | 3> {
  ...
  const existingPid = /^[0-9]+$/.test(existingPidRaw.trim()) ? Number.parseInt(existingPidRaw.trim(), 10) : NaN;
  ...
  const started = deps.startLauncher('/bin/bash', [options.paths.launcher], options.paths.fallbackLog);
...
async function readLogTail(deps: RouterEnsureDeps, logPath: string, lineCount = 40): Promise<string[]> {
  const raw = await deps.readFile(logPath);
  if (!raw) return [];
  const lines = raw.split(/\r?\n/);
  return lines.slice(Math.max(0, lines.length - 1 - lineCount), lines.length - 1).filter((line) => line.length > 0);
}
...
async function bestEffortCopilot(deps: RouterEnsureDeps): Promise<void> {
  ...
  try { await deps.ensureCopilot(); } catch { /* Copilot is an optional fallback. */ }
}

export async function runRouterEnsure(deps: RouterEnsureDeps, options: RouterEnsureOptions): Promise<RouterEnsureResult> {
  let lockOwned = false;
  const removeExit = deps.setExitHandler(() => lockDirCleanup(options.paths.lockDir, lockOwned));
  try {
    ...
    if (launchdResult === 0) { await bestEffortCopilot(deps); return { status: 'healthy-launchd', exitCode: 0 }; }
    if (launchdResult === 1) {
      return {
    ...
    if (fallbackResult === 0) { await bestEffortCopilot(deps); return { status: 'healthy-fallback', exitCode: 0 }; }
    if (fallbackResult === 3) {
      return {
