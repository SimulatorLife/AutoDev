...
export function isAntigravityModel(input: string): boolean {
  try {
    ...
    const candidate = value && typeof value === 'object' && !Array.isArray(value) ? (value as Record<string, unknown>).model : null;
    const model = typeof candidate === 'string' ? candidate.trim().toLowerCase() : '';
...
export function resolveAntigravityEnsureOptions(env: NodeJS.ProcessEnv = process.env): AntigravityEnsureOptions {
  ...
  return {
    ...
    launcher: join(codexHome, 'hooks', 'run-codex-antigravity-proxy.sh'),
    cliPath: env.AGY_CLI_PATH?.trim() || join(home, '.local', 'bin', 'agy'),
    settingsPath: env.AGY_SETTINGS_PATH?.trim() || join(home, '.gemini', 'config', 'config.json'),
    readyTimeoutMs: positiveInteger(env.AGY_READY_TIMEOUT_MS, DEFAULT_TIMEOUT_MS),
    logPath: join(env.TMPDIR?.trim() || '/tmp', 'codex-antigravity-proxy.log'),
  };
}
...
function defaultDeps(options: AntigravityEnsureOptions): AntigravityEnsureDeps {
  ...
  return {
    ...
    settingsValid: () => {
      try {
        const value: unknown = JSON.parse(readFileSync(options.settingsPath, 'utf8'));
        const userSettings = value && typeof value === 'object' && !Array.isArray(value)
          ? (value as Record<string, unknown>).userSettings
          : null;
        return userSettings !== null && typeof userSettings === 'object' && !Array.isArray(userSettings)
...
async function waitForProbe(deps: AntigravityEnsureDeps, timeoutMs: number): Promise<boolean> {
...
export async function ensureAntigravityProxy(
  ...
  if (!deps.settingsValid()) {
    console.error(`Antigravity settings missing or AI Credit Overages is not disabled: ${options.settingsPath}`);
    return 1;
  }
  if (!deps.cliAvailable()) {
    console.error(`Antigravity CLI not installed at ${options.cliPath}.`);
    return 1;
  }
  if (await deps.probe()) return 0;

  if (deps.launchd.isLoaded(options.label)) {
    try { deps.launchd.kickstart(options.label); } catch { /* fall through to readiness */ }
    if (await waitForProbe(deps, options.readyTimeoutMs)) return 0;
    console.error(`Antigravity launchd service ${options.label} did not become ready.`);
    return 1;
  }
