...
export function codexLikeEnv(): NodeJS.ProcessEnv {
  return { HOME: process.env.HOME ?? homedir(), PATH: "/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin", ...(process.env.CODEX_HOME ? { CODEX_HOME: process.env.CODEX_HOME } : {}) };
}

export async function probe(options: ProbeOptions): Promise<ProbeResult> {
  const child = spawn(options.launcher, [ options.server ], { cwd: options.cwd, env: options.env ?? codexLikeEnv(), stdio: [ "pipe", "pipe", "pipe" ], detached: true });
  let stderr = "";
  let buffer = "";
  let nextId = 0;
  ...
  const rpc = (method: string, params: JsonRecord): Promise<JsonRecord | null> => new Promise((settle) => {
    if (exited) { settle(null); return; }
    const id = ++nextId;
    const timer = setTimeout(() => { pending.delete(id); settle(null); }, options.timeoutMs);
    pending.set(id, (message) => { clearTimeout(timer); settle(message); });
    child.stdin.write(`${JSON.stringify({ jsonrpc: "2.0", id, method, params })}\n`);
  });
  const result: ProbeResult = { tools: [], calls: [], exited: null, stderrTail: "" };
  try {
    const init = await rpc("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "autodev-mcp-probe", version: "1" } });
    if (init) {
      child.stdin.write(`${JSON.stringify({ jsonrpc: "2.0", method: "notifications/initialized" })}\n`);
      const listed = await rpc("tools/list", {});
      result.tools = (listed?.result?.tools ?? []).map((tool: JsonRecord) => String(tool.name));
      for (const call of options.calls) {
        const reply = await rpc("tools/call", { name: call.tool, arguments: call.args });
        const content = reply?.result?.content;
        const text = reply === null
          ? (exited ? "no reply: the server exited" : "no reply: timed out")
          : reply.error ? `error: ${JSON.stringify(reply.error)}` : Array.isArray(content) ? content.map((part: JsonRecord) => part.text ?? `[${part.type}]`).join("\n") : JSON.stringify(reply.result);
        result.calls.push({ tool: call.tool, ok: reply !== null && !reply.error && !reply.result?.isError, text: text.length > EXCERPT ? `${text.slice(0, EXCERPT)}…` : text });
      }
    }
  } finally {
...
function parseArgs(argv: string[]): ProbeOptions | null {
  ...
  let server = ""; let cwd = process.cwd(); let timeoutMs = 60_000;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--cwd") cwd = argv[++index] ?? cwd;
      ...
      const raw = argv[index + 1] && !argv[index + 1]!.startsWith("--") ? argv[++index]! : "{}";
      if (!tool) return null;
      calls.push({ tool, args: JSON.parse(raw) as JsonRecord });
    } else if (!arg.startsWith("--")) server = arg;
  }
  if (!server) return null;
  return { server, cwd, calls, timeoutMs, launcher: join(codexHome, "hooks", "run-autodev-mcp.sh") };
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const options = parseArgs(process.argv.slice(2));
  if (!options) {
    process.stderr.write("usage: mcp-probe.ts <server> [--cwd DIR] [--call <tool> '<json-args>']... [--timeout-ms N]\n");
    process.exit(2);
  }
  const result = await probe(options);
  process.stdout.write(`tools (${result.tools.length}): ${result.tools.slice(0, 40).join(", ")}${result.tools.length > 40 ? ", …" : ""}\n`);
  for (const call of result.calls) process.stdout.write(`${call.ok ? "OK  " : "FAIL"} ${call.tool}: ${call.text}\n`);
  if (result.exited) process.stdout.write(`SERVER EXITED code=${result.exited.code} signal=${result.exited.signal}\n--- stderr tail ---\n${result.stderrTail}\n`);
